...
<Steps>
  ...
  <Step title="Foreach participant — compute type, skip self, resolve identity">
    ...
    3. **Identity resolution:** Calls `mvp/identity/check-identity-email` with the participant's email and `user_id`. If a `master_person_id` comes back, use it. If not, compose a fallback name from the email local-part (split on `@`, first piece, `.` → ` `, capitalize), run it through `mvp/format/name-format`, then call `mvp/get-add/master-person-new` (#13039) to create the record.
  </Step>
...
The event-row creation and master_event resolution have no top-level try/catch — if those fail, the whole worker call aborts and `#195` catches it at the per-event level. The participant loop wraps each iteration in its own `try_catch` so one bad participant (e.g. a malformed email, a downstream crash in `get-add/master-person-new`) logs to `log_crash` and continues rather than losing all remaining participants for the event.
...
| `mvp/get-add/master-person-new` (#13039) | Creates a master_person when the identity lookup misses |
...
graph TD
    ...
    subgraph "#2208 history-parse_v3"
        ...
        W_EXISTS -->|Yes| W_TAIL
        W_EXISTS -->|No| W_CREATE["format + get-add/master-person-new (#13039)"]
