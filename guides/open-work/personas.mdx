...
- **Track B — Personas (this page)** sits *on top* of Track A. The person-persona rollup function reads from `person_has_expertise` (and other enrichment sources) to build the per-person 8-dimensional profile. **Built and running.**
- **AlloyDB matcher** — Personas are populated and exported to AlloyDB with ScaNN indexes, running the parallel-CTE matcher pattern from [Find investors — AlloyDB build](/guides/open-work/vectors-alloydb/find-investors-engine). **Done.**
...
| `identity_profile_vector` | json | 3072-d float array (gemini-embedding-001) |
...
3. **`outcome/build-ideal-persona`** orchestrator:
   ...
   - 8 × Gemini `gemini-embedding-001` embeddings
...
table "outcome_personas" {
  ...
  schema {
    ...
    json identity_profile_vector { description = "3072-d vector via gemini-embedding-001" }
    json domain_focus_vector { description = "3072-d vector" }
    json soft_skills_vector { description = "3072-d vector" }
    json track_record_vector { description = "3072-d vector" }
    json public_engagement_vector { description = "3072-d vector" }
    json communication_style_vector { description = "3072-d vector" }
    json community_affiliation_vector { description = "3072-d vector" }
    json passions_interests_vector { description = "3072-d vector" }
...
table "person_personas" {
  ...
  schema {
    ...
    json identity_profile_vector { description = "3072-d vector" }
    json domain_focus_vector { description = "3072-d vector" }
    json soft_skills_vector { description = "3072-d vector" }
    json track_record_vector { description = "3072-d vector" }
    json public_engagement_vector { description = "3072-d vector" }
...
The `vectors/create-gemini-vectors` function (#13114) is the Gemini `gemini-embedding-001` wrapper (3072-d), used across the node and expertise embedding writers.
...
CREATE TABLE person_personas (
  ...
  -- 8 narratives ... text
  identity_profile_vector     vector(3072),
  domain_focus_vector         vector(3072),
  soft_skills_vector          vector(3072),
  track_record_vector         vector(3072),
  public_engagement_vector    vector(3072),
  communication_style_vector  vector(3072),
  community_affiliation_vector vector(3072),
  passions_interests_vector   vector(3072),
...
- **8 Gemini `gemini-embedding-001` calls** — one embedding per persona dimension
...
At 100K people: roughly **$200–400 in one-time synthesis cost** for the entire person corpus. Negligible.

Per-row storage (Xano `json` vector column):
- 8 vectors × 3072 floats × ~8 bytes serialized JSON ≈ **~192 KB per row**
- 100K people ≈ **19.2 GB** in Xano. Acceptable. (AlloyDB with `vector(3072)` is more compact: ~12 KB × 8 = ~96 KB per row, ~9.6 GB at 100K.)
...
| End-to-end synthesis test | ✅ done | Validated against sample outcomes |
| `person/build-persona` rollup | ✅ done | Reads `person_has_expertise` + enrichment sources |
| `passions_interests` LLM cut | ✅ done | Dedicated extraction prompt |
| AlloyDB schema migration | ✅ done | Follows ScaNN pattern from find-investors-engine |
| AlloyDB matcher SQL function | ✅ done | Parallel-CTE matcher |
...
```json
{
  "row": {
    ...
    "identity_profile_vector": [<3072 floats>],
    "...": "...",
...
3. **All 8 vectors are 3072-element float arrays.**
