...
| --- | --- | --- | --- | --- |
| 4645 | `mvp/signature/email-signature-parse-process` | Extract signature \+ enrich person/company (monolith) | `moonshotai/kimi-k2.6` | parse helper Branch C (first-time email address) |
...
<Card icon="play">
  ...
  **Monolith worker.** Skip-guard: returns `"processed"` when `person_enrich_data.email_signature` is already non-empty. Otherwise fetches the person's 3 most recent messages from Nylas, runs a ~200-line content/link-extraction lambda, and calls `moonshotai/kimi-k2.6` (temperature 0.2, `top_p` 0.1, `max_tokens` 800, `data_collection: deny`) to emit structured JSON: `signature_found`, `name`, `title`, `company`, `company_website`, `company_domain`, `phone` (e164/local/country), person/company social links, `calendar_url`.
