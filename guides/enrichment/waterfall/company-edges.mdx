...
- `(Company:VC_Firm)-[:FOLLOW_ON_INVESTED_IN]->(Funding_Round)` · weight **20** — non-lead where an `OPTIONAL MATCH` finds an earlier round the same firm backed (lexicographic `announced_date` compare on ISO strings; strict `<` so re-runs don't self-count). Correct classification depends on **chronological processing**: #12703 v2.3 (2026-07-02) now dispatches a firm's rounds `date asc` so earlier rounds are already in the graph when a later one resolves — out-of-order processing previously misclassified genuine follow-ons as first-time `INVESTED_IN`.
...
**Xano artifact — do not reimplement.** #12701 now JSON-encodes/double-quotes every Cypher string property (v1.9, 2026-07-02 — apostrophe-safe); #12702 still splices investor names and descriptions into Twig-templated Cypher with no escaping (an apostrophe in an org name crashes that item). Parameterize queries in a port.
...
- **Multi-founder companies** use a hub pattern, idempotent by delete-then-CREATE each run: `(Person)-[:FOUNDED]->(cf:Entity:contextFounded)` · weight **5** per founder, plus `(cf)-[:WAS_FOUNDED]->(Company)` · weight **1**. The `contextFounded` hub node carries an LLM founding narrative \+ embedding — if the narrative or embedding call fails, **no founding edges are written that run**.
- **Single founder**: direct `MERGE (Person)-[:FOUNDED]->(Company)` · weight **5** (description "founded" vs the hub's "co-founded").

**Accepted by design (Mark, 2026-07-02).** The hub keeps the founding story vector-searchable (`name_embedding` on the node — direct edges couldn't carry one). The §3.6 audit's purge intent is **superseded**; the only defect was the sub-1 weight, fixed in #12700 v3.2 (`WAS_FOUNDED` 0 → **1**, the identity-strength floor — founder→company path distance is 5\+1, effectively matching a direct w5 `FOUNDED`). Pre-v3.2 hubs still carry w0 and self-heal on re-enrichment via the delete-then-recreate; `tool/backfill-purge-was-founded` #13064 should **not** be run.
...
| `WAS_FOUNDED` | `(contextFounded)→(Company)` | 1 | #12700 | ✅ Live (accepted by design; w0→1 in v3.2, 2026-07-02) |
