...
**The `current_needs` asymmetry.** In **direct** compares, the `current_needs` diagonal is **excluded from the composite**: two people seeking the same thing are competitors for the same resources, not complements — "both raising a Series C" is a reason _not_ to introduce. Direct diagonal drivers are therefore `domain_focus` / `public_engagement` / `passions_interests`. In **simulated** compares, `current_needs` is the _heart_ of the signal — the archetype's needs were built to mirror the target, so a candidate matching them is seeking exactly what the target offers. Needs still serve direct matching where they belong: through the **off-diagonal cross-cells** (the target's `current_needs` against a candidate's `domain_focus`/`public_engagement`/`passions_interests` — the candidate's world answering the target's need), which also remain spike-eligible. A direct `current_needs`×`current_needs` spike **never** qualifies for the spike lane. The raw diagonal cosine is still _saved_ in `compare_persona` (interpretation is this step's job, not storage's) and stays available for observability.
