...
The following block defines the complete `import_nylas_contact` mirror table. Paste it into a DBML-compatible schema tool, then connect `user_id`, `nylas_grant_id`, and `master_person_id` to the corresponding tables in the target database.

```dbml
Enum nylas_contact_mirror_status {
  active
  deleted
}

Enum nylas_contact_processing_status {
  pending
  processing
  processed
  failed
...
Table import_nylas_contact {
  ...
  nylas_object text [not null, default: 'contact']
  source text [not null, default: 'address_book']

  raw_contact jsonb [not null, default: '{}']
  ...
  physical_addresses jsonb [not null, default: '[]']
  im_addresses jsonb [not null, default: '[]']
  web_pages jsonb [not null, default: '[]']
  groups jsonb [not null, default: '[]']
  picture_url text

  mirror_status nylas_contact_mirror_status [not null, default: 'active']
  mirror_revision bigint [not null, default: 1]
  ...
  processed_by_orbiter boolean [not null, default: false]
  update_required boolean [not null, default: false]
  processing_status nylas_contact_processing_status [not null, default: 'pending']
  processing_attempts integer [not null, default: 0]
  processing_locked_at timestamptz
  processing_locked_by text
  next_attempt_at timestamptz
  processing_error text
  processed_revision bigint
  processed_payload_hash text
  processed_at timestamptz
  master_person_id bigint

  indexes {
    (user_id, nylas_grant_id, nylas_contact_id) [unique, name: 'import_nylas_contact_identity_uq']
    (processing_status, next_attempt_at, updated_at, id) [name: 'import_nylas_contact_work_idx']
    (nylas_grant_id, source, last_seen_sync_id) [name: 'import_nylas_contact_reconcile_idx']
    (user_id, mirror_status) [name: 'import_nylas_contact_user_status_idx']
  }
}
```

<Info>
  DBML does not portably represent the PostgreSQL partial predicate on `import_nylas_contact_work_idx`. After generating PostgreSQL, replace that full work index with the partial index defined in [Constraints and indexes](#constraints-and-indexes). Also update `updated_at` from application code or a database trigger; its DBML default only covers insertion.
</Info>
