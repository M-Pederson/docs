...
<Check>
**Default seed behavior (cascade_depth: 0):** calling `get-add/master-person-new` (#13039) with `cascade_depth: 0` gives you a **fully enriched person AND their primary current company**, synchronously dispatched. _(The previous entry point `get-add/master-person` #12553 was deprecated 2026-06-01; all callers migrated to #13039.)_

- The person runs through `enrich-master-person` (all 12 phases; legacy `run-base-person-enrich` remains compatible).
- Phase 3 (Process PDL) identifies the best current role — ranked **founder → C-suite → VP → latest start year** — and calls `get-add/master-company-new` with `cascade_depth: 0` and `queue: false` hard-coded (v3, 2026-04-15). This fires the current company get-add enrichment and `enrich-master-company` immediately for that one company, regardless of the enclosing person's depth.
...
Set or confirm these before flipping either task to `active = true`; the depth caps should be encoded as guards in #126 / #127 or in the orchestrators they dispatch:

- **Person full-enrichment depth:** choose the maximum `cascade_depth` that should run `enrich-master-person` (recommended starting policy: people at depth **0–1** only).
- **Company full-enrichment depth:** choose the maximum `cascade_depth` that should run `enrich-master-company` (recommended starting policy: companies at depth **0–2** only).
- **Primary current employer exception:** keep `process-person-phase-3`'s `cascade_depth: 0`, `queue: false` current-employer path enabled; current employers should receive full company enrichment even when the person was discovered deeper in the cascade.
- **Depth overflow behavior:** decide what happens beyond the caps before activation — leave rows queued for manual review, mark them deferred, or delete them with a `log_crash` audit row. Do not silently full-enrich depth-3+ rows.
...
- **Seed-person dispatch hold:** if live seed-person enrichment is intended, remove the `debug.stop "HOLD!!"` in `get-add/master-person-new` (#13039) separately from queue activation. Queue activation does not affect that seed-path hold.
</Warning>
