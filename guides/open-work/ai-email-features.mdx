...
Forward-looking spec for the rebuild, numbered for reference. The Existing Functions inventory below is the as-is baseline.

### R1 — Global "Enable AI Features" default toggle (default: on)

A single **Enable AI Features** toggle sits at the top of the user account interface where accounts are connected. It defaults to **true** and acts as the global default state for AI email processing.

- When an email account is connected and appears in the UI, the account row carries its **own toggle, initialized to mirror the global setting**.
- The per-account toggle can be overridden independently; the override wins for that account.
- Backing today: the per-account toggle maps to `nylas_grant.ai_enabled`. The global default is new — a user-level setting that seeds each account's flag at connect time.

**Preprocessing behavior** — the account's effective toggle state at the time the email account is connected governs what preprocessing does:

| Effective state at connect | Preprocessing |
|---|---|
| **On** (default) | Email bodies are saved, every email is cleaned to a `clean_email_body`, and signatures are extracted — all during preprocessing. |
| **Off** | No email bodies are saved, no emails are cleaned, and no signatures are extracted. |

**Toggle lifecycle** — the per-account toggle is locked until processing completes:

1. The account connects and preprocessing starts immediately with the effective state inherited from the global default — e.g. global on → the entire import table and all of its records are built with AI features enabled. **Processing is triggered automatically when preprocessing finishes.**
2. Through both stages — preprocessing and processing — the per-account toggle stays **disabled or hidden**: the user has no chance to enable or disable it away from the global default until processing is complete.
3. Only once processing is complete is the toggle presented again, and the user can reverse the choice: turning AI features **off** triggers the scrub/delete endpoint (R2); turning AI features **on** after an AI-off import triggers the backfill API (R3).

<Note>
  To decide: whether flipping the global toggle later re-syncs account toggles that were never manually overridden, or only seeds accounts connected after the change. (Enable-later on an AI-off import is now covered by R3.)
</Note>

### R2 — AI-off purge API

Turning AI features **off** for an account that has completed processing is not just a flag flip: a **separate, dedicated scrub/delete API** runs immediately and deletes, for that account, **every cleaned email body (`clean_email_body`) and every saved raw email body**.

<Note>
  To decide: whether the purge also covers artifacts derived from those bodies — extracted signatures, thread summaries, Mem0 memories — or only the bodies themselves.
</Note>

### R3 — AI-on backfill API

The reverse path: the import table was created with AI features **off**, all of the account's emails were processed from it (email events exist, but no bodies were saved), and the user then turns AI features **on**. A **separate API** works from the **existing email events** for that account and **adds** the raw email bodies and their `clean_email_body` for each.

<Note>
  To decide: whether the backfill also runs signature extraction (which an AI-on preprocessing would have done), or adds bodies only.
</Note>
