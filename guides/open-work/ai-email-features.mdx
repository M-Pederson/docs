...
*Workshop space — one AI pass per thread for summaries, Mem0 memories, and email tone. All five forks below are decided (2026-07-22); distillation into a requirement is pending.*

**The problem.** Three LLM jobs read the same threads, each its own way. Summaries are per-thread but only within the AI Processing Depth (90 days). Mem0 extraction is full-history but **per-contact** — a thread with Jason and Josh Diamond gets re-fetched, re-cleaned, prefiltered, and re-extracted once *per contact*. The tone stage runs its own Nylas sent-mail sweep. And #12644 re-fetches from Nylas and recomputes on every call even when nothing changed.

**Recommendation 1 — make the thread the unit of AI work.** One thread-processor pass produces everything derivable from a thread: the summary (if in-window), memory candidates for **every resolvable participant**, and sent-sample harvesting for tone corpora. Multi-subject extraction replaces per-contact re-processing: the extractor emits each memory with subject attribution (Jason / Josh / both) and the writer fans out to each participant's Mem0 scope. Prior art: #13149 already resolves `surfaced_contacts` / `retrieval_contact_ids` on every memory — this extends "annotate related people" into "attribute subjects."

**Recommendation 2 — a thread ledger makes re-processing cheap to refuse.** Extend the email count tables with per-`(grant_id, thread_id)` state: latest message id + message count, `summarized_at`, `memories_extracted_at`, `tone_harvested_at`, and prompt/model versions. A thread re-enters the AI pass only when its message set changed or a prompt/model version bumped. Prior art: the tone stage's `corpus_hash` skip in `email_draft_tone` (same idea, generalized to threads), `email_ai_summary`'s `(grant_id, thread_id)` upsert key, and Mem0's `canonical_memory_hash` / `run_id` duplicate checks — which stay as the last line of defense, not the first.

**Recommendation 3 — one Nylas read feeds everything.** The import already stages bodies (R2). Summaries, memories, and tone should consume the staged bodies / `clean_email_body` store instead of re-fetching: today #12644 pulls up to 50 messages per thread from Nylas and Mem0 runs per-contact `any_email` searches. With staged bodies, the AI pass is a database scan, not an API sweep.

**Recommendation 4 — split by depth, share the cleaning.** In-window threads (within 90 days): summary + memory extraction from the same cleaned transcript — potentially one combined call that emits the summary plus memory candidates, or at least a `memory_worthy` flag replacing today's separate prefilter call. Out-of-window threads (Mem0 goes full history): no summary; deterministic cleaning of the staged raw bodies (today's Mem0 lambda — no LLM cleaning cost) feeding the cheap prefilter → extract chain.

**Sequencing (decided — capture-then-apply).** Extraction never waits for IDs; writes always have them. **Preprocessing** stages everything user-side: per-address artifacts on the **import table** (signature JSON per R7, tone per R8) and per-thread artifacts on the **email_events / thread ledger** (summary, `memory_worthy`, and memory candidates attributed by participant email address). **Processing** then creates each contact from its import record and, at that moment, applies the staged artifacts — signature and tone from the import record, and the thread candidates attributed to that address written to Mem0 with the real `contacts_id` \+ `master_person_id`. Mem0 is never written before the contact exists. The R4 scrub deletes staged candidates with the rest of the user-side artifacts, and the lazy full-history deep pass appends older threads to the same ledger and staging. **Write mechanics (decided): stage-then-send, never early Mem0 writes.** Candidates are staged keyed by `(thread_id, participant_email)` and sent to Mem0 at contact **create-or-resolve** during processing, with full `contacts_id` \+ `master_person_id` metadata from birth (`participant_email` carried into metadata for retrieval and scrub filtering). Early Mem0 writes under provisional email-address keys were considered and rejected: the contact read path is `contacts_id`-keyed so early memories are unreadable until patched, patching costs the same number of API calls as sending, it would add a Mem0 update-path dependency the pipeline doesn't have, addresses that never become contacts would leave permanent orphan memories, and scrubbing external writes is far harder than deleting staged rows.

Open implementation detail: candidates as explicit rows keyed `(thread_id, participant_email)` (plain-query join and scrub — leaning this way) vs. a JSON array on the event record.

**Being workshopped:** (1) *decided:* memories extract **automatically, bounded by the AI Processing Depth** — same 90-day window as summaries, one shared pass; a contact's **full history** runs lazily the first time that contact's memories are opened (user-triggered deep pass; the thread ledger prevents rework); (2) *decided:* **one-pass multi-subject extraction** — the extractor attributes each memory to its subject(s) and the writer fans out to every resolvable participant's scope; shared threads are never re-processed per contact; (3) *decided:* **the summary call doubles as the prefilter** for in-window threads — it also emits a `memory_worthy` flag (+ reason), and extraction runs only on worthy threads with its own specialized prompt; out-of-window threads keep the cheap standalone prefilter; (4) *decided:* **staged bodies are the single transcript source** — the AI pass is a database scan driven by the thread ledger; Nylas is touched only by the lazy full-history deep pass and, later, the webhook path; (5) *decided:* **capture-then-apply sequencing** — extraction in preprocessing, Mem0 written only at contact creation during processing (see Sequencing above).
