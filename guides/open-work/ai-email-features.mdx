...
2. **Scrub scope vs. shared enrichment.** Bodies, thread summaries, and account-derived Mem0 memories are account-scoped and delete cleanly. Signature-derived enrichment is **not** account-scoped — it lands on shared person/company records (`person_enrich_data.email_signature`, work history, links) visible to other users. *Recommendation: R4 deletes account-scoped artifacts only; shared person-level enrichment stays.* Needs an explicit call.
...
4. **Ongoing mail after import.** These requirements govern the historical import. A forward-path requirement is missing: does each new message arriving via webhook after the account is Ready follow the account's *current* AI state (save + clean at receipt)? Today the live webhook chain runs no AI steps at all (see First-pass observations, item 11).
