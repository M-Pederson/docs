...
- **The §11 tier coercion is load-bearing (and violates the house `first_notnull` rule):** `priority_tier|first_notempty:4` coerces an explicit tier `0 → 4` — live behavior a parity port must reproduce, flagged not fixed. (The matching deal-cascade depth coercion in #12856/#12702 was **fixed 2026-07-02**: `cascade_depth` is now a nullable input read via `first_notnull`, so an explicit seed `0` survives.)
...
- **`add-company-node` #2602's null-domain fallback no longer sets `is_vc: true`** — repaired in v1.7 (2026-07-02); a domainless company gets only the placeholder logo. Rows mislabeled by the pre-v1.7 branch are not backfilled (open backfill candidate; details on [Company Nodes](/guides/enrichment/waterfall/company-nodes)).
