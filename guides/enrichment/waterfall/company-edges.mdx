...
This page is the **pipeline-scoped catalog**: which edges this pipeline emits, at what weight, from which writer — including a per-edge **live status**, because one whole edge family is currently unreachable from the live pipeline (see the [investor edge set](#investor-edge-set-12702--as-written-vs-live)). Edge patterns, properties, and examples should be edited only in the ontology source of truth. Writer internals live on [Company Functions](/guides/enrichment/waterfall/company-functions); node MERGE keys on [Company Nodes](/guides/enrichment/waterfall/company-nodes).
...
`(Company)-[:RAISED]->(Funding_Round)` · weight **10**. Written by `add-fundable-deal-node` (#12701) from the per-deal cascade (#12856), after the `Funding_Round` node MERGE (node key = the Fundable deal id — see [Company Nodes](/guides/enrichment/waterfall/company-nodes)). Edge `created_at` coalesces to the deal's **announced date** (epoch millis at UTC midnight) when present, else write time; `uuid` is deterministic (`<companyNodeUuid>-<dealId>`). Silently skipped when the raising company has no graph node yet. Cypher: [Cyphers → Funding](/guides/cyphers/funding). This is the **only investor-side edge that materializes live** — see below.

### Investor edge set (#12702 — as-written vs live)

<Warning>
  **As-built (flagged):** `resolve-investors-edges` (#12702) **self-skips on every live invocation** since v1.11 (2026-06-23). Both #12856 and #12702 default `cascade_depth` via `first_notempty:1`, and Xano's `first_notempty` treats int `0` as empty — so an explicit seed `cascade_depth: 0` coerces to 1, #12856's `effectiveDepth == 0` gate is unsatisfiable, and #12702's own `> 0` guard always fires (returning `"skipped_depth_limited"` after a `qa_passed: true` log row `SKIP: resolve-investors-edges gated to seed depth` whose `data.cascade_depth` reads **1** even on seed calls — the runtime fingerprint). Net live behavior: the deal cascade materializes **Funding_Round \+ RAISED only**; every edge in this section — and the co-investor / partner / angel queueing beside it — is dead code. A `first_notnull` swap alone would not restore intent (the declared input default is already 0, so missing ≡ explicit 0 — the guard needs a nullable input). A Go port must decide explicitly: strict parity reproduces the coercion; intent parity must not.
</Warning>

The **as-written contract** (unreachable live, everything else about it verified from code): for each institutional investment on the deal, #12702 adds the `VC_Firm` label to the investor's Company node and MERGEs one of:
...
```mermaid
graph LR
  PC["Company (portfolio co)"] -- "RAISED w10 — LIVE" --> FR["Funding_Round"]
...
*Dashed edges = as-written but unreachable live (#12702 self-skip).* The pre-drift docs also omitted `FOLLOW_ON_INVESTED_IN` and `INVESTMENT_PARTNER_AT` from this set entirely. Type-flip hazard as-written: a lead/non-lead change or a later `INVESTED_IN → FOLLOW_ON_INVESTED_IN` reclassification MERGEs a second edge of the new type without deleting the old one.
...
| `LEAD_INVESTED_IN` | `(Company:VC_Firm / Person:Angel)→(Funding_Round)` | 15 | #12702 | ⚠️ Unreachable (self-skip) |
| `INVESTED_IN` (institutional) | `(Company:VC_Firm)→(Funding_Round)` | 20 | #12702 | ⚠️ Unreachable (self-skip) |
| `INVESTED_IN` (angel) | `(Person:Angel)→(Funding_Round)` | 25 | #12702 | ⚠️ Unreachable (self-skip) |
| `FOLLOW_ON_INVESTED_IN` | `(Company:VC_Firm)→(Funding_Round)` | 20 | #12702 | ⚠️ Unreachable (self-skip) |
| `INVESTMENT_PARTNER_IN` | `(Person)→(Funding_Round)` | 15 | #12702 | ⚠️ Unreachable (self-skip) |
| `INVESTMENT_PARTNER_AT` | `(Person)→(Company:VC_Firm)` | 20 | #12702 | ⚠️ Unreachable (self-skip) |
