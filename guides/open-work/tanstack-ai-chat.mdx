...
no Outcomes Submit Outcome, dispatch, upload, or contact-card action has been
clicked yet, and the next live click still requires explicit action-time
approval.
...
`git diff --check`, Mintlify validate, and `CI=true pnpm build` after the
build script retried the known intermittent Nitro public-assets race.
