...
*Workshop space — one AI pass per thread for summaries, Mem0 memories, and email tone. Nothing here is decided yet.*
...
**Sequencing.** Memories and the tone join need contacts to exist, and contacts are created during processing — so the AI pass lands after processing: preprocessing (bodies \+ clean \+ signatures) → processing (graph \+ contacts) → AI pass (summaries \+ memories \+ tone join), all under the R3 per-account job lock, driven by the thread ledger.

**Being workshopped:** (1) *decided:* memories extract **automatically, bounded by the AI Processing Depth** — same 90-day window as summaries, one shared pass; a contact's **full history** runs lazily the first time that contact's memories are opened (user-triggered deep pass; the thread ledger prevents rework); (2) *decided:* **one-pass multi-subject extraction** — the extractor attributes each memory to its subject(s) and the writer fans out to every resolvable participant's scope; shared threads are never re-processed per contact; (3) *decided:* **the summary call doubles as the prefilter** for in-window threads — it also emits a `memory_worthy` flag (+ reason), and extraction runs only on worthy threads with its own specialized prompt; out-of-window threads keep the cheap standalone prefilter; (4) *decided:* **staged bodies are the single transcript source** — the AI pass is a database scan driven by the thread ledger; Nylas is touched only by the lazy full-history deep pass and, later, the webhook path.
