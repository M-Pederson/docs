...
**Being workshopped:** (1) *decided:* memories extract **automatically, bounded by the AI Processing Depth** — same 90-day window as summaries, one shared pass; a contact's **full history** runs lazily the first time that contact's memories are opened (user-triggered deep pass; the thread ledger prevents rework); (2) confirm one-pass multi-subject extraction; (3) combined summary+memory call vs. separate stages for in-window threads; (4) confirm staged bodies as the single transcript source.
