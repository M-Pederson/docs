...
2. **Scrub scope vs. shared enrichment.** R7 moves the signature JSON to user-side import records, so it deletes cleanly under R4. Remaining call: signature-derived *enrichment* (work rows, links, company records) still lands on shared person/company records visible to other users — does that still happen in the rebuild, and is it exempt from the R4 scrub? *Recommendation: shared enrichment stays (graph facts are shared knowledge); R4 deletes user-scoped artifacts only.*
