...
| 3 | **Apply LLM payload** | Only when `$llmRan`. Calls `apply-company-llm-payload` #12989 — the same shared helper used by fresh creates (see the Note on [Entry Points → Phase 5](/guides/enrichment/waterfall/company-entry-points#phase-5-record-creation-section-10), including the `company_financial` overwrite flag). |
