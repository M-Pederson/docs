...
2. **Generate markdown via unstructured.io.** Convert the uploaded file to markdown and save the markdown to the same `files` row.

3. **Run the `classify_file` LLM.** Produces `file_description` and `file_type`; persist both back onto the `files` row.
