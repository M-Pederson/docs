...
1. **Global flip → existing accounts.** Does changing the global toggle re-sync per-account toggles that were never manually overridden, or only seed future connections? *Recommendation: future connections only — per-account state changes only by explicit per-account action. Least surprising, and no hidden mass scrubs or backfills.*
