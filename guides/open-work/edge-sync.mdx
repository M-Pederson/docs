...
| Edge | Target | Nature |
|---|---|---|
| `KNOWS` | Person | Evidence-based, carries relationship strength |
...
Declared `CONNECTED_ON_*` edges are binary facts and coexist with KNOWS on the same pair — KNOWS is the relationship spine. A LinkedIn import creates **all three**: the person stub, `CONNECTED_ON_LINKEDIN`, and a KNOWS edge whose sole evidence is the declared connection — initialized at the **weakest weight (~100**, under the graph convention *weight 1–100, lower = stronger*). Real interaction evidence strengthens it (weight falls) from there.
...
- **CREATE-CONTACT-PERSON** — the sole way a person node enters. Triggered by user intent: outbound email (via the three-tier lookup / GET-ADD), **meeting participation** (attended/accepted calendar event, small-meeting cap applies — see the FOLLOWS gate decision), a vCard/CSV/LinkedIn import, or a manual add. Creates the person stub + `(User)-[KNOWS]->(Person)` + the membership row + the mem0 kickoff. Import-created KNOWS starts at the weakest weight (~100, lower = stronger) with the declared connection as its only evidence — plus the matching `CONNECTED_ON_*` platform edge.
...
### Path stitching across graphs — the Ian Andes case

The question that tests the whole model: the user is connected to Ian Andes **only** in their private graph (`CONNECTED_ON_ORBITER`); Ian's `WORKS_AT → NVIDIA` edge exists **only** in Universe; NVIDIA is not in the user's graph, and Universe holds no path from the *user* to NVIDIA. The user searches "Nvidia" — where does the path *user → Ian → NVIDIA* come from?

**Neither graph produces it. The read service assembles it, joined on the shared `node_uuid`:**

1. **Search** — "Nvidia" against both graphs: user graph has nothing; Universe returns the NVIDIA node (`uuid N`). One merged candidate.
2. **Anchors** — the service pulls the user's anchor set from the private graph: every `node_uuid` with a relationship edge from the user. Ian (`uuid I`) is in it — **the anchor set is edge-type-agnostic**: a `CONNECTED_ON_ORBITER`-only relationship anchors exactly like a strong KNOWS (it just ranks lower).
3. **Continuation** — Universe is asked one question, and it is *not* "path from user to NVIDIA": it is **"traverse outward from NVIDIA ≤k hops; which endpoints are in this anchor list?"** It finds `NVIDIA ←WORKS_AT— Ian`, matches `uuid I`, returns the segment. The user's own presence in Universe is never consulted — paths start at anchors, not at the user.
...
Each hop renders with its tier and evidence ("connected on Orbiter, no interaction history yet" → "currently works at NVIDIA"). This is the normal case, not an edge case: hop 1 exists only privately and hops 2+ exist only publicly for essentially every path the product computes — the shared `node_uuid` is the entire join, which is why the merge-repoint invariant is load-bearing.

**Anchor-set boundary for paths**: intro-path anchors are the **relationship edges only** — KNOWS, `CONNECTED_ON_*`, FOLLOWS, WORKS_AT, ATTENDED. `LOCATED_IN` and `COLLECTED` edges are context and ranking signals, not path anchors — a city cannot make an introduction.
