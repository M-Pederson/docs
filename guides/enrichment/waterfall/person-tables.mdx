...
Function behavior lives on [Person Waterfall Functions](/guides/enrichment/waterfall/person-functions); graph writes on [Nodes](/guides/enrichment/waterfall/person-nodes) and [Edges](/guides/enrichment/waterfall/person-edges); the queue's replacement drain requirement under [Build Drain Crons](/guides/enrichment/waterfall/enrichment-crons#build-drain-crons).

<Note>
  **Last reviewed & updated: 2026-07-09** — schemas and indexes re-verified against live workspace 3 (branch `v1`) via the Metadata API.
</Note>
...
<Note>
  ...
  **`log_crash` is also the canonical kill-switch audit trail** (since 2026-06-30): #13039 removed the `kill_switch_blocked_people` side-write — blocked-person events land here and nowhere else.
</Note>
...
**The four index gaps** — the headline parity notes for a rebuilt store, all verified live:
...
4. **`education_experience` #230 has no `master_person_id` btree** — unlike its sibling `work_experience` #147, which has one; every education-resolver read scans (PK \+ `created_at desc` only).
...
### Who writes here (controlled waterfall)

Under the 2026-07-04/05 controlled-waterfall policy person runs write **`queue_enrich_company`** again — but only as depth-1 leaves (past employers via `resolve-edges-work` #12562, sparse non-primary Exa employers via #13031); schools and the selected current company **full-enrich inline instead of queueing**. The full story is on the [overview](/guides/enrichment/waterfall/person-waterfall). `queue_enrich_person` rows come from a short producer list:
...
| `resolve-investors-edges` #12702 (company-side deal cascade) | Co-angels \+ VC partners on a deal — driven by the company waterfall's deal walker (#12703 → #12856); the person phase 9 now writes direct portfolio edges instead of cascading | Depth 1, tier 1 |
| Company-waterfall seed-only people fanout | YC founders / partner, Fundable investor partners discovered by a **seed company** run | Depth 1, tier 1 |
...
flowchart TD
    ...
    D --> E
    E --> F["Task #126 tick (600s): pick 1 row<br/>WHERE processing = false AND priority_tier ≤ 1<br/>sorted priority_tier, cascade_depth, id"]
    ...
    H -->|crash| J["Release: processing = false + log_crash"]
```

<Warning>
  **Nothing drains this queue automatically today.** The only scheduled drain, cron **Task #126 `process-person-queue`**, ships `active = false` (freq 600s, datasource live). When active it processes **one `priority_tier <= 1` row per tick** (max 6 people/hour — the 2026-07-03 tier gate leaves lower-priority rows to tier escalation or the operator drain), sorted `priority_tier asc, cascade_depth asc, id asc`, locking `processing = true`, dispatching `{master_person_id, deep_research, cascade_depth}` into #13040, and deleting the row only after success. Replacement inventory: [Build Drain Crons](/guides/enrichment/waterfall/enrichment-crons#build-drain-crons).
</Warning>
...
| 27 | `social_insights` | text **list** | nullable, trim | `get-social-insights` #12669 v1.26 (`x-ai/grok-4.5` via #13190 v2.1; two bounded Exa evidence searches, semantic validation, one bounded retry) |
...
| --- | --- |
| `exa_search` | Exa search-lane stash (separate from the cascade's `exa` match object) — since 2026-07-06 also carries `fiber_email`, the audited Exa Fiber fallback attempt (#13039 v1.15) |
...
| --- | :-: | --- | --- | --- |
| `master_link` | #166 | `create-master-link` #2541 (v1.8) | **Lowercased** normalized `link_url`, scoped per owner (v1.7/v1.8 — the same URL can attach to a Person and separately to a Company) | The best-indexed table in the family (btrees on both FKs). See below for scraper fan-out |
...
`create-master-link` #2541 normalizes, lowercases, and dedups the URL per owner, then its per-domain branches **launch async scraper enrichment**: LinkedIn (→ ScrapeCreators \+ sets `master_person.linkedin_url`), Twitter/X (→ `scrape-twitter-person` #2106 v1.11), Crunchbase, YouTube, Facebook, IMDB (person `imdb.com/name` links dispatch `add-imdb-to-master-person` #10509 synchronously since v1.5). Since **v1.3 (2026-06-22)** a `suppress_downstream` input guards **all 9** async dispatch sites, and #13039 v1.10 passes it whenever `hold_downstream` is set or `cascade_depth > 0` — scraper fan-out is suppressed on contained paths.
...
- **`replace-avatar` #237 (v4.12, 2026-07-08)** — priority-based replacement: a candidate only displaces the current `main` when its source ranks higher. Live text-source priority order: **Signal NFX \> LinkedIn / ScrapeCreators LinkedIn \> Serper \> Fundable / other \> Twitter** (v4.2 dropped the legacy `data_source` int input — attribution is text-source-only). The v4.9–v4.12 wave blocks provider placeholder images, LinkedIn cover/background banners, and LinkedIn company logos from becoming main person avatars, and makes duplicate-URL source attribution priority-aware.
...
Phase 9 reads two mirror tables directly: `fundable_people` #667 (unique `linkedin_person_id` \+ `crunchbase_person_id` slugs; `master_person_id` backlink written by `link-fundable-person` #12793 and the seed task #155) and `fundable_angel_investments` #679 (angel person→deal join, btrees on `person_id` / `deal_id`, no unique pair). Since the 2026-07-02 D-series rework each angel-investment row becomes a **direct `(Person)-[:INVESTED_IN w25]->(Company)` edge** (default-on for depth-0 angels; `allow_investor_network_cascade: false` suppresses) — the full deal cascade `cascade-deal-participants` #12856 runs from the **company side** only. Full mirror-family schemas: [Company Waterfall Tables → Fundable mirror family](/guides/enrichment/waterfall/company-tables#fundable-mirror-family).
...
**Writer:** `upsert-person-has-expertise` #12925 (v1.2, 2026-07-02 — fixed the two-arg `|concat` mis-render, added the dedupe guards), called by `resolve-person-expertise-v2` #12926 (v2.8) after every `HAS_EXPERTISE` MERGE. Also written by the music credit-expertise emitter path (#13160 → #12926), which the unique pair keeps idempotent. Since 2026-07-06, `llm-identify-person-expertise` #12666 **clears the person's rows here** alongside its `HAS_EXPERTISE` edge delete on every re-resolution, so the table mirrors the current graph edge set.
...
Work rows from every provider — the input `resolve-edges-work` #12562 and `choose-best-current-role` #12621 read. Written by phase 3 (PDL), `process-enrich-layer` #12615, phase 4b #13031 (Exa — routed through `add-position-record` #12608 since v1.7 so mixed-source duplicates update instead of multiplying), and `process-full-enrich` #13049 (deduped on write). Notable columns: `company_name` / `company_domain` / `company_linkedin_url` (the resolution keys), **text-typed** `start_day/month/year` \+ `end_day/month/year` (do not parse as ints blindly), `current` (bool), `edge_uuid` (graph writeback from `create-work-edges` #2800 — a null `edge_uuid` is retryable, and #12562 v1.13 copies the canonical edge uuid onto duplicate source rows), `triples_created_at`, dual provenance, and two self-documented deprecations (`main` — "depreciated??", `linkedin_company_id` — "depreciated!!"). btree on `master_person_id`.

### `education_experience` (#230)

Education rows — the `resolve-edges-education` #12560 input (also written by phase 4b #13031 v1.12, which materializes Exa `educationHistory` here). `school_name` / `linkedin_url` / `school_domain` resolution keys, **list-typed** `field_of_study` / `degree_name` / `activities_societies`, int-typed `start/end_month/year` (unlike #147's text dates), `edge_uuid` writeback, `master_company_id` set once the school resolves. **No `master_person_id` btree** (index gap #4) — PK \+ `created_at desc` only.
