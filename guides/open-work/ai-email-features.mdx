...
<Card icon="database">
  ...
  **Booking-link table (person-scoped, shared).** Where the enrich half writes a parsed signature's `calendar_url`: `master_person_id` (→ `master_person`) \+ `scheduling_url`. The prompt admits booking links only (calendar-*event* links return null). No unique key exists on `(master_person_id, scheduling_url)` and #10519 inserts without the dedup query the retired #4645 had — duplicates are possible until the rebuild adds one. Used in the **processing** phase.
</Card>
...
6. **Resolved — #4645 retired (2026-07-22):** signature prompt and enrichment logic are now single-sourced in the split pair. Carry-over: #10519 still lacks the `scheduling_link` dedup fix #4645 had.
