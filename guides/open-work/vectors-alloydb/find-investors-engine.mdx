...
Stores unstructured.io's element-level output for each file — one row per parsed element (Title, NarrativeText, Table, ListItem, Image, etc.) with page number, position, and original metadata preserved. Sibling to the `file` table, joined on `file_id`. The pitch pipeline (`pitch/extract-pitch-profile`) reads only `file.markdown` and is unchanged by this table — element rows are additive, unlocking opt-in upgrades without forcing a migration.
