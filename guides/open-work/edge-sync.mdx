...
Worked example — **Katelyn** is a member of two orgs, **GrantDriveGroup** and **Orbiter.io**. Her active mode selects the source set:

- **Company mode (GrantDriveGroup active)** — she searches *"Founders Collective"* → **three searches in parallel**: her user graph, the **GrantDriveGroup org graph** (the mode-selected one — Orbiter.io's org graph is not consulted), and Universe. Await all three → merge & dedupe by `node_uuid` → compute paths from the merged candidate set.
...
- **Multi-org selection**: a user may belong to many orgs, but a query reads **exactly one org graph** — the one the active mode selects. Never a union of orgs.
- **Await-all is bounded**, not open-ended: each tier runs under its latency budget from the SLO decision (user 50ms · org 100ms · Universe 300ms / 1.5s), and a tier that breaches degrades per the private-first ladder — the response ships with tier-provenance labels saying what's missing.
...
7. **Universe read scaling.** Per the SLO decision: the separate private-tier FalkorDB deployment, at least one Universe read replica with replica-first routing, the circuit breaker + per-tier latency budgets in the read service, server-side query timeouts everywhere, and the celebrity-node query policy as the graph grows.
