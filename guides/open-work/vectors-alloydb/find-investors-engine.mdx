...
**Index swap: HNSW → ScaNN.** This page originally specified pgvector HNSW indexes for every vector column. We're flipping every vector index to AlloyDB's `alloydb_scann` extension *before any DDL is run*.
...
**What changes in your tasks on this page**

1. **Step 1 DDL — Clone the table in AlloyDB (section starts ~line 431, vector indexes ~lines 530–545).** All six `CREATE INDEX … USING hnsw (vec vector_cosine_ops) WITH (m=16, ef_construction=64)` statements become `CREATE INDEX … USING scann (vec cosine)`. Add `CREATE EXTENSION IF NOT EXISTS alloydb_scann CASCADE;` at the top of the DDL block (it cascades the `vector` extension automatically). Drop the `vector_cosine_ops` operator class — ScaNN takes the distance keyword (`cosine` / `l2` / `dot_product`) inline in the column list.
2. **`file` and `file_chunk` HNSW indexes (lines 248, 378).** Same swap.
3. **Match-engine SQL — Strategies B and C (sections at lines 1243 and 1477).** No structural change — ScaNN is a drop-in for the index choice. The `<=>` cosine distance operator at query time is unchanged. **Net new capability:** the match function can now add `WHERE` clauses on metadata columns (sector, stage, geo, expertise_uuids) and the planner will push the filter into the ScaNN traversal automatically. No special syntax needed to opt in — just write the `WHERE` clause.
4. **Tuning section (~lines 1500–1505) and Gotchas (~lines 1716–1722).** Replace HNSW knobs (`m`, `ef_construction`, `hnsw.ef_search`) with ScaNN equivalents. Index params: `num_leaves` (auto-tuned with `MODE=AUTO` — recommended default), `quantizer` (`SQ8` default; `AH` preview for higher recall; `FLAT` for exact). Query-time tuning lives under `scann.*` GUCs (see AlloyDB docs). Delete the "ivfflat alternative" line — ScaNN replaces both HNSW and IVFFlat.
5. **Mermaid diagram annotation (line 418).** `+ HNSW indexes` → `+ ScaNN indexes`.
6. **`EXPLAIN ANALYZE` plan examples (~lines 1460–1475).** Index-scan node names will change. Re-capture from a real ScaNN-indexed run after first benchmark.

**What does NOT change (no rework needed)**
...
**Open call before DDL lands.** ScaNN's `MODE=AUTO` auto-tunes `num_leaves` for you and is the recommended default for first builds. Stick with AUTO unless someone has a specific recall/latency target that needs hand-tuned partitions.
...
CREATE INDEX file_pplx_emb_hnsw_idx  ON file USING hnsw (pplx_embeddings vector_cosine_ops);    -- AlloyDB-only
```
...
- HNSW on `pplx_embeddings` — full-document cosine similarity search
...
CREATE INDEX file_chunk_embedding_hnsw_idx ON file_chunk USING hnsw (chunk_embedding vector_cosine_ops);
```
