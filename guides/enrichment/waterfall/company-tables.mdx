...
<Note>
  ...
  **As-built (flagged):** cron Task #127's catch block writes `phase: "queue"`, which is **not in the enum**. `phase` is a strict enum — an out-of-enum write can fail at runtime, which inside a catch block would swallow the crash row *and* skip the `processing: false` release that follows it, wedging the queue row. The task ships `active = false` so this does not fire today. **Xano artifact — do not reimplement:** a Go port should use a valid enum value (or extend the enum), not copy the string.
</Note>
...
2. **Coercion** — `cascade_depth` defaults through `first_notempty:0`; `priority_tier` through `first_notempty:4`. An explicit `priority_tier: 0` is "empty" and **becomes 4** (worst) — tier 0 is unreachable through this function, and the effective tier default is 4, not the declared input default 0. A Go port must reproduce the 0→4 coercion, not honor 0.
...
| 30 | `seeded_by_user` | int | nullable, FK → `user` #125 | Written **raw** (no coercion) — an omitted input lands null. Read by the phase-7 seed-marker gate |
| 31 | `source` | text | nullable, trim | Row provenance — `first_notempty` default `"Base Company Enrich"`. Read by the phase-7 seed gate and the orchestrator's investor-skip gate (`"User Input"`, `"ScrapeCreators Linkedin"`, `"Funding Rounds"`) — do not conflate with `enrich_history_company.source` |
...
| `"Funding Rounds"` | phase-7 #12813 | Only when a fundable org exists AND the seed-fanout gate passes |
...
| --- | --- | --- | --- |
| `apply-company-llm-payload` #12989 §5 | add_or_edit | `company_type`, `is_public`, `ticker`, `stock_label`, `primary_exchange`, `stock_link`, `went_public_on`, `funding_total`, `total_rounds_raised`, `revenue` | Only when `company_llm.data` non-empty. Called from #12558 §10 (fresh creates) and #12992 Step 3 |
| phase-6 #12810 | edit-else-add | `funding_total`, `total_rounds_raised`, `is_public` | Only when a `fundable_organizations` row backlinks this company |
...
1. **Step 3 — #12989 writes the LLM view (`funding_total` with the deterministic Fundable fallback: LLM `funding.total_raised_usd` `|first_notempty:` `round(fundable.total_raised × 1,000,000)` when finite and \> 0, v1.5), last-write-wins.** **As-built (flagged):** the §5 upsert writes every listed field **unconditionally** — there is no `first_notempty` against the existing `company_financial` row (the function header's "preserves existing values" claim is true for `master_company` scalars only). A re-run with an emptier LLM payload nulls out previously good values. `company_type` coalesces against the **constant** `"Privately Held"`, not the stored row — a null payload resets a stored type. v1.6 suppresses `funding_total` to null for `"Venture Capital Firm"` rows (fund size/AUM is not company financing).
2. **Step 8 — phase-3 YC** force-writes `company_type: "Private - VC Funded"` over the LLM type for YC companies.
3. **Step 10 — phase-6 re-stomps.** **As-built (flagged):** #12810 then **unconditionally overwrites** `funding_total`, `total_rounds_raised`, and `is_public` with the Fundable view whenever a fundable org is linked — Fundable wins over LLM for those three fields on every run. Its lambda is `total_raised || 0`, so it writes `funding_total.value = 0` (not null) when Fundable lacks a total — stamping a zero over an LLM value, and re-introducing a `funding_total` on VC firms that #12989 deliberately suppressed. (The old doc's phase-6 claim — "canonicalize profile URLs" — was wrong: #12810 has zero profile-URL logic; it is the domain-only Fundable back-link repair plus this financial overwrite.)
...
| `industry_join` | #307 | `add-company-industry-array` #2547 (arrays); `add-industry` #137 (single) | #2547: per-company name-level, **case-insensitive**, new lookup rows stored **lowercase**. #137: none — always inserts a join row | ⚠️ Asymmetry: #137 stores lookup rows as-given, which #2547's lowercased `db.get` then never matches — duplicate mixed-case `industry` lookup rows are live behavior; flag, don't silently merge. Intra-array dupes in one #2547 call also leak (the existing-set isn't updated mid-loop) |
...
| `processed_at` | timestamp | **Xano-added** — stamped by `cascade-deal-participants` #12856 when the deal is materialized (node \+ edges). **Null = needs processing; non-null = skip** — the re-run idempotency gate |
