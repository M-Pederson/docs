...
**Content edges** (User → …): `CREATED` → Note · `RECEIVED` → Note (shared in) · `UPLOADED` → File (manual) · `SAVED` → File (auto-captured from email) · `OWNS` → Project *(suggested, with the tier)*

**Context edges** (leaf → leaf): `ABOUT`: Note → Person | Company | Project · `ATTACHED_TO`: File → Person | Company | Project
</Info>
