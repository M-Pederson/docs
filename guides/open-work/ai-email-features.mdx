...
**Toggle lifecycle** — the per-account toggle is locked while preprocessing runs:

1. The account connects and preprocessing starts immediately with the effective state inherited from the global default — e.g. global on → the entire import table and all of its records are built with AI features enabled.
2. While preprocessing runs, the per-account toggle is **disabled or hidden** — it cannot be changed mid-import.
3. When the import table is complete, preprocessing is finished: the toggle is presented again and the user can reverse the choice — e.g. turn AI features off for that account.

<Note>
  To decide: (a) whether flipping the global toggle later re-syncs account toggles that were never manually overridden, or only seeds accounts connected after the change; (b) what happens when AI is enabled later on an account imported with AI off — its emails have no saved bodies, so backfill vs. forward-only needs a call.
</Note>

### R2 — AI-off purge API

Turning AI features **off** for an already-connected account is not just a flag flip: a **separate, dedicated API** runs immediately and deletes, for that account, **every cleaned email body (`clean_email_body`) and every saved raw email body**.
