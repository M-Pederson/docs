...
```text
mvp/enrich/enrich-master-person   — #13040   (v3.10, 2026-06-02)   ← active orchestrator, dispatched by master-person-new and process-full-enrich
...
| **3** | `process-person-phase-3` #12824 | **Process PDL** — skills, interests, languages, education, work, certs, bios. **v3.2 (2026-06-03):** primary-current-company lookup is now non-recursive: it calls `get-add/master-company-new` with `queue: true`, `cascade_depth: 1`, and `priority_tier: 1`, then links `master_person.master_company_id` / `current_title`. v3 (2026-04-15) had fully enriched the current employer inline; v3.2 intentionally stops that company wave. |
