...
| **3** | `process-person-phase-3` #12824 | **Process PDL** — skills, interests, languages, education, work, certs, bios. **v2 (2026-04-14):** also enriches the primary current company immediately (finds best current role, calls `get-add/master-company` with `queue=false`). |
...
As of v2 of `process-person-phase-3` (2026-04-14), once PDL is processed the phase immediately finds the person's best current role and calls `get-add/master-company` with `queue=false` — this **enriches the primary current employer synchronously at depth 0** rather than queuing it.
...
│
│       Phase 3 (process-person-phase-3 v2) — Process PDL
│       │   + auto-enrich primary current company (queue=false)
