...
3. **Xano-driven `investment_thesis` build writes to AlloyDB with embeddings.** Confirm a Xano function call with a `master_person_id` or `master_company_id` produces an `investment_thesis` row in AlloyDB with all narrative embeddings populated. Test cases: Scott Belsky (person path), Signafire / Accel / a16z (VC path).
