...
(8 files / 71 tests), scoped Biome (30 files), frontend/docs
`git diff --check`, guard searches, Mintlify validate, and `CI=true pnpm build`
from the frontend pre-commit build hook. Phase 10 desktop in-app browser
...
- Local validation after the handoff update passed: JSON parse for the new
  manual-click artifact, focused tests (8 files / 71 tests), scoped Biome
  (30 files), frontend `git diff --check`, docs `git diff --check`, Mintlify
  validate, the Meeting Prep event-loading guard, and the narrowed legacy/AI SDK
  guard.
