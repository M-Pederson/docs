...
Proposed success criterion: after the scrub, the account's stored data is indistinguishable from an account originally imported with AI off.
...
Proposed success criterion: after the backfill, the account is indistinguishable from an account originally imported with AI on. Backfill is necessarily best-effort — messages deleted upstream since the import cannot be re-fetched.
...
3. **Parity for derived artifacts.** If the R4/R5 success criteria above are adopted, the derived-artifact questions answer themselves: scrub also removes thread summaries and account-scoped memories; backfill also runs signature extraction and summaries, not just bodies. Confirm parity is the intent.
