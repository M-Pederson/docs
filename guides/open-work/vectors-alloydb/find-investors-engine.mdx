...
This step lifts the deck markdown into a structured pitch profile — Layer 1 metadata (company, stage, founders, raise amount, customers, runway, capital split, etc.) plus **6 narrative dimensions** (founder_fit, problem_market, competitive_moat, traction_momentum, business_model, expansion_roadmap) — and upserts a row into `fundraising_pitch_profiles` with **6 × 1536-d embedding vectors** (one per narrative). The vectors are what the AlloyDB matcher SQL function compares against investor-thesis vectors.

**Orchestrator:** `pitch/build-pitch-profile` — Xano workspace 3, function id `12918`. Wraps the extraction LLM with 6 × `openai/text-embedding-3-small` embedding calls (one per narrative dimension) and a guarded `db.add_or_edit` write keyed on `master_company_id` > `master_person_id` > `db.add` fresh.

**Extraction LLM:** `pitch/extract-pitch-profile` — Xano workspace 3, function id `12917`. **Model:** `deepseek/deepseek-v3.2` via OpenRouter (~$0.27/M input, $1.10/M output). **Observed:** ~30s end-to-end (LLM + 6 embeddings + DB write) on a 15K-char deck; markdown is truncated to the first 60K chars before send.
