...
Used for IMDB identity disambiguation. Replaced `google/gemini-2.5-flash` (verify-imdb-url) on 2026-04-06. `llm-identify-person-expertise` #12666 (which had replaced `anthropic/claude-sonnet-4` here on 2026-04-05) moved to `google/gemini-3.1-flash-lite` on 2026-06-19 — see the next section.
...
Used for expertise signal extraction and structured name parsing. `llm-identify-person-expertise` #12666 moved here on 2026-06-19 with `reasoning: {enabled: false}` and `max_tokens: 2500` — the previous effort-low config produced intermittent empty content; its v5 prompt emits per-signal `start_year`/`end_year`, and results feed `resolve-person-expertise-v2` #12926 (the person's existing `HAS_EXPERTISE` edges are cleared before the rewrite; `resolve-person-expertise` #12668 is legacy, no longer in the call path). `name-format` #2649 (v3.4, 2026-05-30) runs strict `json_schema` structured output with the response-healing plugin and `require_parameters: true`.

| Function | Temp | Max Tokens | Timeout | Reasoning | Avg Input Tokens | Avg Output Tokens | Cost/Call | Updated |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `llm-identify-person-expertise` #12666 | 0 | 2500 | 30s | disabled | _TBD_ | _TBD_ | _TBD_ | 2026-06-19 |
...
graph TD
    ...
    A --> D["#12666 llm-identify-person-expertise<br/>gemini-3.1-flash-lite · temp 0 · reasoning off"]
