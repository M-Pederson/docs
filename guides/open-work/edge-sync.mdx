...
| `WORKS_AT` / `FOUNDED` / other person→company edges | Company | User's own **full employment & founder history** — all 1-hop Universe edges copy at sign-up; `is_current` + `valid_from`/`valid_to` (null = current) distinguish present roles |
...
1. **New user sign-up** — **all of the user's own 1-hop Universe edges copy** (full employment & founder history via WORKS_AT/FOUNDED/etc. with `is_current` + `valid_from`/`valid_to`, education via ATTENDED, locations via LOCATED_IN, plus imported/connected people); **expertise nodes are the one exclusion** (they stay Universe-side).
...
```cypher
(Kyle)-[:WORKS_AT {is_current: true, title: "Founder"}]->(:Company {name: "Being Human"})
(Kyle)-[:WORKS_AT {is_current: false, title: "Chief Executive Officer"}]->(:Company {name: "Talespin"})
(Kyle)-[:WORKS_AT {is_current: false, title: "Co-Founder"}]->(:Company {name: "OpenDrives"})
...
```mermaid
graph LR
    K((Kyle Jackson)) -->|WORKS_AT is_current · Founder| BH[Being Human]
