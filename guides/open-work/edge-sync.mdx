...
- **CREATE-CONTACT-PERSON** — the sole way a person node enters. Triggered by user intent: outbound email (via the three-tier lookup / GET-ADD), a vCard/CSV/LinkedIn import, or a manual add. Creates the person stub + `(User)-[KNOWS]->(Person)` + the membership row + the mem0 kickoff.
- **CREATE-CONTACT-COMPANY** — the sole way a company node enters. Triggered by user↔company-domain interaction *(gated on the seeding-gate decision below)*, a manual add or import, or the user's own enrichment (below). Creates the company stub + `(User)-[FOLLOWS]->(Company)` — or `WORKS_AT` when it is the user's own employer.
...
### The user's own enrichment

When the user's own person node completes enrichment, their professional core lands in their graph automatically: every **current** company (present employment, not historical) and their education, as stubs with the respective edges. Example — Mark, fully enriched:

```cypher
(Mark)-[:WORKS_AT {current: true}]->(:Company {node_uuid: "…", name: "Orbiter"})
(Mark)-[:ATTENDED {end_year: 2004}]->(:School {node_uuid: "…", name: "University of Minnesota"})
```

```mermaid
graph LR
    M((Mark)) -->|WORKS_AT current| O[Orbiter]
    M -->|ATTENDED| U[U. of Minnesota]
    M -->|KNOWS strength 87| S[Sam Seely]
    M -->|FOLLOWS| K[Knock.app]
    M -->|CREATED| N1[Note stub]
    N1 -->|ABOUT| S
```

Historical employers stay in Universe — readable at compose time, never materialized. The point of materializing the current core: these stubs join the **anchor set**, so alma-mater paths ("you and this founder both attended…") and colleague paths start from the user's own graph like every other anchor.

### Notes and documents — nodes, not edges *(recommended 2026-08-08 — confirm to lock)*
...
```cypher
(User)-[:CREATED]->(:Note {note_id, created_at})-[:ABOUT]->(Person | Company)
(User)-[:UPLOADED]->(:Document {doc_id, mime, name})-[:ATTACHED_TO]->(Person | Company)
```

**Content never lives in the graph.** The graph node is a stub — ids, timestamps, an embedding pointer at most. Note text lives in Backend (AlloyDB); document bytes live in GCS. FalkorDB is in-memory: the graph stays an index of relationships, and stubs resolve to content at hydration time. The payoff is graph-RAG retrieval: "everything I know about Sam" is a single 1-hop traversal — KNOWS edge, notes ABOUT him, documents ATTACHED_TO him — with each stub hydrated from its home store.
...
6. **FOLLOWS company-contact flow.** CREATE-COMPANY-CONTACT seeding FOLLOWS edges from user↔company-domain interaction. *Gated on the seeding-gate decision below.*
...
- **FOLLOWS seeding gate.** Company FOLLOWS edges are proposed to seed from interactions with people at a domain — but person-contact creation is decided **outbound-only**, and inbound-driven FOLLOWS would re-open the spam/newsletter pollution that rule closed. Define: interaction threshold, outbound-only vs accepted-calendar-invite intent, role-domain blocklist. Gates scope item 6.
...
- **Notes/documents model confirmation.** The nodes-not-edges stub model above is recommended, not yet locked — confirm, then it moves into the Decision card.
