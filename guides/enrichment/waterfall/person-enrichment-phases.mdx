...
<Note>
All Phase 7 `resolve-edges-*` functions were upgraded in April 2026 to accept `cascade_depth` and pass `queue: true` + `priority_tier` + `source_function` + `source_entity_id` to every `get-add/master-company` and `get-add/master-person-new` (#13039) call. This makes the cascade traceable in the queue tables.
...
- PDL + EL fire inside `get-add/master-company` at initial create time
