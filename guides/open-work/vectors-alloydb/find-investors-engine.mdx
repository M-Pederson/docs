...
The current schema embeds the whole `file.markdown` as one `pplx_embeddings` vector. That works as long as the markdown fits inside `perplexity/pplx-embed-v1-4b`'s context window (verify the actual limit on OpenRouter).
