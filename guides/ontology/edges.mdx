...
2. **KNOWS is the only dynamically scored user-graph edge** — computed by the Known model (evidence-based; internals not documented here). Declared-only creation (LinkedIn import, in-app connect) initializes at the weakest tier (≈98–100); interaction evidence lowers it.
...
|---|---|---|---|
| `KNOWS` | `(User)-[:KNOWS]->(Person)` | **Dynamic 1–100** (Known model; declared-only init ≈98–100) | CREATE-CONTACT-PERSON — all twelve [entry points](/guides/open-work/edge-sync) |
