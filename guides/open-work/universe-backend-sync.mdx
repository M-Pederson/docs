...
A user emails `mrx@gmail.com`:

1. Backend checks its own tables first — `contacts_*` for this user, then `enrichment_*` (the three-tier lookup in the canonical flow below). Only if **both miss** does the address go to Universe.
2. GET-ADD seeds the address in Universe with **user provenance**.
3. If the address is used for a PDL lookup and **PDL returns a result**, the address is now independently confirmed by enrichment: we null the user provenance on the master row and set `source = <enrichment name>`.
4. The address is now enrichment-tier and freely shareable.
5. If it never resolves via any enrichment, it stays user-provenance only — private.
...
- ~~**Tier-3 CREATE-CONTACTS with nothing to copy.**~~ **Dissolved 2026-08-08 by the [User Graph Nodes & Edges](/guides/open-work/edge-sync) reference-model decision** — CREATE-CONTACTS never copies a subgraph; it creates a stub + KNOWS edge from the GET-ADD response alone, which works identically for brand-new and fully-enriched people.
- ~~**Enrichment-update fan-out to user graphs.**~~ **Resolved 2026-08-08 by the [User Graph Nodes & Edges](/guides/open-work/edge-sync) decision** — user graphs never copy shared edges, so there is nothing to fan out; enrichment updates are visible to every user's next composed query the moment Universe writes them. The only surviving propagation is the merge/node-event channel (stub repointing on Universe merges).
- ~~**Is the user-graph copy a duplicate or a reference?**~~ **Decided 2026-08-08 — reference, on [User Graph Nodes & Edges](/guides/open-work/edge-sync)**: user graphs hold only the private layer (thin stubs by `node_uuid` + KNOWS/FOLLOWS/WORKS_AT + strength + notes/files); all shared topology stays in Universe; reads compose the layers app-side. CREATE-CONTACTS step 3 accordingly shrinks from "copy subgraph" to "create stub + KNOWS edge."
...
- **What do inbound emails do?** Contact creation is outbound-only (decided). Presumably an inbound email from an existing tier-1 contact still LOG-INTERACTIONs (a reply from a contact is relationship evidence), and inbound from an unknown sender is ignored entirely — confirm both halves. **Partial update 2026-08-08 ([User Graph Nodes & Edges](/guides/open-work/edge-sync) FOLLOWS-gate decision): meeting participation — attending/accepting a calendar event with a person — is now the second contact-creating intent event alongside outbound email (small-meeting cap applies). Passive inbound remains a no-op.**
