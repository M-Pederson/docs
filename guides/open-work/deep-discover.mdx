...
| film_tv | `imdb_url` | thin title seed (open build — see film row above) | `imdb_url` |
...
<Note>
**Build needed: an API to add a title from a `tmdb_id`.** TMDB discover returns some titles before they carry an IMDb id (common for unreleased/upcoming titles), and today those fall to leads. A thin endpoint that takes a `tmdb_id`, resolves the IMDb id via TMDB `/movie/{id}/external_ids` (re-checking later when it's not yet assigned), and feeds the same title seed would make every TMDB result seedable — and it's the natural home for the film-row title seed above.
</Note>
