...
Every person enters the waterfall through **one synchronous get-add gate** — `mvp/get-add/master-person-new` #13039. The gate parses, dedups (twice — a cheap pre-API pass, then a 4-tier post-cascade resolution), enriches via the PDL → Enrich Layer → Exa Fiber → Exa cascade, and creates inline; new depth-0 people with `queue == false` dispatch the [phase orchestrator](/guides/enrichment/waterfall/person-functions) immediately (async), while cascade leaves discovered at depth \> 0 land in `queue_enrich_person` for the future [drain CRON](/guides/enrichment/waterfall/enrichment-crons#build-drain-crons) to process. Architecture spec: [Person Waterfall overview](/guides/enrichment/waterfall/person-waterfall) · function roster: [Person Waterfall Functions](/guides/enrichment/waterfall/person-functions) · table schemas: [Person Waterfall Tables](/guides/enrichment/waterfall/person-tables).

## Live Xano function inventory

**Audit source:** Xano workspace `3`, branch `v1`. The inventory starts from the live entry gate `#13039`, orchestrator `#13040`, queue drainer `#12816`, FullEnrich tier `#13036` / `#13049`, and company C-suite seed path `#12997`, then includes active literal `function.run` callees plus production sidecars called out in the live Person roster. Legacy/deprecated/sandbox functions are excluded. Adjacent Company, IMDB, and Music waterfalls are listed only as explicit handoff targets, not recursively expanded.
...
| Person graph, contact, and media helpers | `mvp/node/register-node-registry` #13183 | node_registry writer (gate v1.21 stamps new Person nodes) |
...
| Adjacent IMDB and Music handoffs | `mvp/enrich/kickoff-cross-waterfalls-from-expertise` #13185 | reusable film/music bridge — called by the deep-research webhook #8303 and the IMDB/Music orchestrators after expertise re-runs |
...
**Active inventory count:** 121 Xano functions plus endpoint `#8594`. Queue-drain Task `#126` remains inactive; its queue table is listed under [Build Drain Crons](/guides/enrichment/waterfall/enrichment-crons#build-drain-crons). Legacy, deprecated, and sandbox functions are not part of this entry-path inventory.
...
<Card icon="play">
  ...
  **v1.21 (2026-07-09) — THE entry point.** Get-or-create for `master_person`: audit row \+ LLM name split #2649 → kill switch `check-kill-switch-v2` #4617 (ON = local-lookup-only, all new creation blocked; blocks audited to `log_crash` #542 only) → **pre-API local dedup** (v1.6) → provider cascade #13037 (PDL → Enrich Layer → **Exa Fiber email fallback** → PDL retry → Exa; **depth-0 only**) → 4-tier resolution #13038 → either the existing-person path (attach new contacts, return with `person_created: false`) or the new-person path (`master_person` row \+ graph node #2601 \+ `node_registry` stamp #13183 \+ name writes #12820 \+ `person_enrich_data` stash \+ contact write loops \+ Fundable link/sync) → downstream dispatch (exactly one): `hold_downstream` → nothing; new \+ depth 0 \+ `queue == false` → **async** `enrich-master-person` #13040; else queue upsert #12854. When the cascade reports thin email-only data (`needs_full_enrich`), it also fires the async [FullEnrich tier](#fullenrich-async-tier) #13036.

  **Containment inputs:** every `create-master-link` call carries `suppress_downstream: true` whenever `hold_downstream` is set or `cascade_depth > 0` (v1.10) — contained paths store profile links without launching the async platform scrapers. **Recent hardening (v1.14 → v1.21, 2026-07-06 → 07-09):** the Exa Fiber fallback tier with `source: "Exa Fiber"` attribution, `human_review_required` rows for `promoted_pending_review` Fiber identities, zero-width/non-breaking-character name normalization before any `master_person` write, an explicit `person_created` response flag on every path, and `node_registry` registration (`entitiy_type: "master_person"` — the live column name carries that typo) for every Person node the gate creates. The full section-by-section walkthrough follows below.
</Card>

## Calling contract

**Current version:** v1.21 (2026-07-09). THE single get-add of the person waterfall — every function caller and the V2 `POST /master-persons` endpoint were repointed to it on 2026-06-01 (the legacy monolith `master-person` #12553 and old orchestrator `run-base-person-enrich` #12554 have since been **deleted** from workspace 3); the other ways person work starts (queue drain, C-suite dispatch, adjacent pipelines, seed tasks) are [catalogued below](#other-ways-person-work-starts).
...
At depth 0 the caller usually has no parsed name — #13039 takes only `full_name` (there are no `first_name` / `last_name` inputs); the pipeline discovers the name from the providers and parses it via LLM. The full live input surface:
...
- New persons are **blocked**, and the block is audited in `log_crash` #542 only (the `kill_switch_blocked_people` #611 side-write was removed in v1.13; no reprocessing task ever drained that table). A legacy predecessor `mvp/stop/check-kill-switch` #2774 still exists live but is not the pipeline gate.
...
**v1.9 (2026-07-06).** Runs only when the pre-API dedup misses **and** `cascade_depth == 0`. Pure data fetch — no DB writes except a `log_crash` on a caught exception:

```text
PDL /v5/person/enrich (email + first/last + linkedin if any)
  │
  │  if pdl.status == 404  ($pdlEmail404 = true)
  │  — or 200 with empty data.experience (thin data continues) —
  ▼
Enrich Layer (Tier 2)
  ...
  │
  │  if PDL thin AND no usable EL LinkedIn AND email + full_name present
  ▼
#13179 api/exa/fiber-person-email  (Exa Fiber verified-email fallback, v1.5–v1.9)
  │  promote its LinkedIn when email_resolved == true and linkedin_url non-empty
  │  → immediately re-run EL person-linkedin on the Fiber LinkedIn (v1.9)
  │  weak candidates → exa_fiber_candidate_links (evidence-only master_link rows)
  │  unverified-but-usable → promoted_pending_review (human_review_required row)
  ▼
PDL retry  /v5/person/enrich  (profile = the EL- or Fiber-derived LinkedIn)
...
#12936 mvp/enrich/get-exa-profile  (Exa Tier 3)
  │
  │  if $usedElEmail AND no LinkedIn resolved
  ▼
needs_full_enrich = true   (caller dispatches FullEnrich after person_enrich_data is created)
```

The cascade returns one object — the provider payloads plus `el_links` (LinkedIn \+ Twitter \+ Facebook URLs, deduped), `el_profile_pic`, `linkedin_url`, `input_email_source`, `needs_full_enrich`, and the auditable `exa_fiber_attempt` object (eligibility, called/promoted booleans, status, and an explicit promotion/no-promotion reason — persisted by the gate under `person_enrich_data.exa_search.fiber_email`). A PDL 200 with empty `experience` is treated as thin data, so the ladder continues through EL / Fiber / retry / Exa instead of stopping. Every tier is wrapped in its own `try_catch` — an Enrich Layer failure doesn't block Exa; failures write `log_crash` and continue. The EL email path uses `lookup_depth=deep&enrich_profile=enrich` (heuristic resolution beyond EL's cache) and rebuilds the LinkedIn URL from `public_identifier`; Exa runs a `people`-category name search (5 results) and slug-filters to the candidate's LinkedIn. The Exa payload is **stashed only** — processing into bios / avatar / `work_experience` happens later in orchestrator Phase 4b (`process-person-exa-profile` #13031).

**What's NOT in the cascade:** no name/phone-only resolution (all tiers require an email or a LinkedIn URL — a phone-only seed falls back to the thin-record path), and no second-pass verification of an EL-derived LinkedIn URL (a wrong EL URL propagates to `master_person.linkedin_url` — watch `log_crash`; Fiber-derived URLs, by contrast, get the `human_review_required` audit lane). Env vars: `PEOPLE_DATA_LABS` (Tier 1), `ENRICH_LAYER` (Tier 2), `exa` (Fiber \+ Tier 3).
...
graph TD
    ...
    B -->|Yes| C["Enrich immediately<br/>enrich-master-person #13040 v3.33<br/>(async-shared)"]
    B -->|No| D["mvp/queue/upsert-enrich-person #12854<br/>(idempotent: count++, min depth,<br/>OR deep_research)"]
```

For Zeno at depth 0 with `queue: false`: **immediate enrichment** fires asynchronously — `enrich-master-person` #13040 (v3.33, 2026-07-05) with `master_person_id`, `deep_research`, `cascade_depth`. For a depth-1 person discovered during a deal cascade: **queued** via `upsert-enrich-person` #12854 (v1.1) with the source function \+ source entity (id \+ uuid \+ type) and priority tier recorded — the already-queued branch lives inside the upsert (count\+\+, min tier, min depth, `deep_research` OR-ed). For QA/backfill creates with `hold_downstream: true`: **nothing** is dispatched or queued, and a `qa_passed` skip row is logged. Queue schema \+ drain build list: [Person Waterfall Tables](/guides/enrichment/waterfall/person-tables) · [Build Drain Crons](/guides/enrichment/waterfall/enrichment-crons#build-drain-crons).
...
<Card icon="globe">
  ...
  **#8594 v1.9 (2026-06-09):** webhook receiver (API group `Full Enrich`, group 1280, canonical `8hTTyqsU`). The **v1.7** wait-loop polls the orchestrator's `enrich_history_person.processing` completion marker so the writeback never races a live run; **v1.8** is the empty-`data_source` refusal guard; **v1.9** threads the C-suite fields through to #13049. Response counters: `{matched_ped, processed, wait_timeouts, skipped, refused_no_data_source}`.

  **#13049 v1.10 (2026-07-04):** the applier — see the tree above; the `force_run: true` re-dispatch bypasses #13040's 60 s debounce, v1.9 filters `employment.all[]` to current roles under `current_company_only` (companies resolve via `master-company-new`, never the legacy company get-add), and v1.10 repairs `master_person.master_company_id` from the resolved current work row before the async rerun.
</Card>
...
The endpoint contract above reflects the live swagger for group 350.
...
`process-person-queue` **Task #126** drains `queue_enrich_person` #582 one row per 600 s tick — gated to `priority_tier <= 1` since 2026-07-03 — and calls #13040 with the stored `master_person_id` / `deep_research` / `cascade_depth`. It **ships `active = false`**; in practice the queue is drained manually via `mvp/queue/process-enrichment-queue` #12816 (v1.2, 2026-07-04 — routed through the gated orchestrators so depth gates apply to queued leaves) or "Run Now" on the task. The replacement drain requirement is listed under [Build Drain Crons](/guides/enrichment/waterfall/enrichment-crons#build-drain-crons).

### Company-seed C-suite dispatch (`current_company_only`)

When `enrich-master-company` #12992 (v4.24, 2026-07-08) Step 7 accepts Exa C-suite/founder matches for a **seed** company — the step is **depth-0-gated** (v4.2) and non-recursive (v4.3: deeper companies stash Exa but never process people); since v4.21–v4.24 it can also synthesize seed-scoped founder matches from `Company_LLM` / Fundable / LinkedIn evidence when Exa misses a verified founder — each person goes through `mvp/get-add/master-person-from-exa` **#12997 v2.5 (2026-07-07)**: normalized-LinkedIn dedup (plus seed-company \+ first/last-name fallback dedupe for slug variants, v2.0), **kill-switch-gated create** (#4617 is called twice — company-resolve containment \+ the person-create gate), `add-person-node` #2601 \+ `create-master-link` #2541, `is_c_suite: true`. **v1.7:** new Exa leaves start `visibility = false`; **v1.8:** employer resolution is lookup-only (`allow_create: false`); **v1.9:** guarantees a `person_enrich_data` shell and backfills empty `exa` payloads from the match; **v2.2/v2.3 (2026-07-06):** the helper now immediately runs Enrich Layer `person-linkedin` on the Exa LinkedIn URL (storing the rich EL profile and EL-discovered links without downstream fanout) and reconciles cached ScrapeCreators employee cards — using the employee-card image as the preferred avatar candidate so C-suite leaves don't keep placeholder avatars; **v2.4/v2.5 (2026-07-07):** exact-match seed-company scoping (no name-prefix false positives) and zero-width-character name normalization.

#12997 writes **no work rows and no `WORKS_AT`-family edges** — after the upsert, #12992 direct-dispatches `enrich-master-person` #13040 async with `{cascade_depth: depth+1, deep_research: false, current_company_only: true}` (since v4.5, 2026-06-09), and the work rows \+ `WORKS_AT`-family edges materialize inside that run (Phase 3 PDL \+ Phase 4b from the stashed Exa — since #13031 v1.14 the selected current company may materialize when Exa provides strong domain/LinkedIn evidence, still `queue: false` \+ `hold_downstream: true`). In this mode the person run filters PDL / Enrich Layer / Exa / FullEnrich employment arrays to **current roles only**, and Phase 7 skips every broad edge resolver **except education** (#12828 v1.3, 2026-07-04 — schools full-enrich even for C-suite leaves; work / cert / project / honor / volunteering resolution stays suppressed) — company-born people cannot spawn a new graph wave.

### Adjacent pipelines that mint people

- **IMDB (Film & TV):** `run-base-imdb-person-enrich` #12880 (v1.10, 2026-07-08), drained from `queue_imdb_person` #706 by Task #159, creates `master_person` \+ `master_link` \+ avatar \+ LLM bios \+ credit edges for film/TV people — and since 2026-07-09 kicks the cross-waterfall bridge #13185 after its expertise pass. Gates \+ contract: [IMDB Waterfall Entry Points](/guides/enrichment/waterfall/imdb-entry-points).
...
| `"Exa Fiber"` | LinkedIn URL promoted (or stored as candidate evidence) by the Fiber verified-email fallback (v1.14–v1.16) |
| `"Fundable"` | Profile URL synced from a matched `fundable_people` row (v1.9) |
| `"IMDb Verify"` | IMDb URL stored by the phase-8 / cross-waterfall verifier when `verify-imdb-url` #12677 returns high confidence |
...
Pass `hold_downstream: true` to skip BOTH the direct #13040 dispatch AND the queue path — a `qa_passed` skip row is logged to `log_crash`. Use it to run #13039 in isolation while step-debugging. Since **`create-master-link` #2541 v1.3 (2026-06-22)** the scraper fan-out is contained too: a `suppress_downstream` input guards **all 9 async scraper dispatch sites** (LinkedIn / Twitter / Instagram / Crunchbase / YouTube …), and **#13039 v1.10** passes it whenever `hold_downstream` is set or `cascade_depth > 0` — contained paths store links silently. `scrape-twitter-person` #2106 (v1.11, 2026-07-05) treats Twitter-expanded profile URLs as person-link evidence only and never creates or queues companies from them.
...
The gate was split out of the monolithic `mvp/get-add/master-person` #12553 in May 2026; all 19 function callers plus the V2 `POST /master-persons` endpoint were repointed on 2026-06-01, and both #12553 and the legacy orchestrator `run-base-person-enrich` #12554 have since been **deleted** from workspace 3 (only frozen `_BACKUP` copies #12821 / #12727 remain). The RocketReach tier was retired on 2026-05-30 (the `person_enrich_data.rocket_reach` column is gone). The full v1.0 → v1.21 version ladder lives in #13039's own header comment in Xano; the historical migration procedure is preserved in [person-pipeline-polish](/guides/robert-mark/person-pipeline-polish).
