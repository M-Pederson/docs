...
<Note>
  **QA tightening shipped (2026-07-09):** `mvp/get-add/master-company-new` #12558 now closes optional Fundable no-hit / school-bypass outcomes as successful terminal skips and applies captured `Company_LLM` payloads on existing-company match paths through `apply-company-llm-payload` #12989. The applicator persists the just-captured payload before landing additive aliases, alternate domains, profile links, contact emails, and phone candidates without overwriting higher-confidence settled fields.
</Note>

<Note>
  **QA warning follow-up closed (2026-07-10):** the VamosWatu `PASS_WITH_WARNINGS` case exposed two warning-only gaps: clean optional-source misses could still look like failures, and a later `Company_LLM` payload could carry useful alias/contact/profile candidates after an existing-company match. The company entry-gate/applicator fix above covers additive payload landing; the downstream person/expertise follow-up now parameterizes `HAS_EXPERTISE` cleanup/writes in #12666/#12926 and closes handled person closeout misses as `SKIP` rows in #13040 v3.35. A clean rerun should not emit warning-only failures for these paths.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Build Drain Crons — Company" icon="clock-rotate-left" href="/guides/enrichment/waterfall/enrichment-crons#build-drain-crons">
