...
<Steps>
  ...
  <Step title="Secondary response → de-dupe → seed → enrich">
    When the deep-discover API responds with its anchored entities, **de-dupe against the base discovery response**: an LLM compares the entity array to the nodes in the initial response and removes anything already present (fold this into the relevance-gate call — same prompt, one extra boolean per entity). Survivors **seed into the Orbiter Universe by type** through the registry entry points, capped at K = 6. Universe-wide de-dupe needs no separate resolver: every seed endpoint is an idempotent get-or-create gate (#12558 domain ladder, #13039/#13038 person match, MBID gates' dedup window, #12874 imdb_url dedup), so an entity already in the graph but missed by RAG simply resolves to its existing record — which is exactly the "link it into the answer" bucket, for free. Then **poll until every seeded entity has finished enrichment** (poll cadence and a hard deadline are an open knob below).
  </Step>
...
| film_tv | `imdb_url` | `mvp/imdb/get-add/imdb-title` #12874 (or Go `POST /film-tv/enrich`) | `imdb_url` |
...
| film_tv | `imdb_url` | ✅ | `mvp/imdb/get-add/imdb-title` #12874 (Go `POST /film-tv/enrich` as alt) | `imdb_url` |
...
**Graded 2026-08-18** (grades + notes on the run rows in table 785): 2 A, 3 A−, 3 B+, 1 B, 2 B−, 2 C+, 1 C, 1 D across the 15 queries. The grades locked three decisions and drove two fixes:

- **Keep-threshold includes 0.5.** The candidate tier repeatedly held the true answer (Reach Music, ClickUp, Wipster, Amusement Records); only `< 0.5` drops, ranked by score within the kept set.
- **Ingest cap K = 6, by score.** Top-6 on A/B-grade queries is consistently ingest-worthy; weak queries self-limit because few entities clear 0.5.
- **Union re-confirmed.** The strongest entities were disproportionately dual-engine hits, and the gap-fill guard logged repeated correct rejections with zero bad upgrades.
- **Fix (v1.8.10, validated run 49):** an unverifiable constraint ("raised funding in 2026") no longer zeroes an otherwise-matching entity — run 32 had zeroed Figure itself; the rerun kept 9/10 at ≥ 0.5.
- **Fix (v1.8.10/11, validated run 51):** credited-role-on-a-title queries ("executive producers of Severance") now route to **zero web lanes** — the shaper returns an empty `target_types` (honored as deliberate, not fail-open) because credits live in the graph and the IMDb pipeline, never on the professional web. Run 40 had returned studio staff instead of the actual EPs.
...
- ~~Union both engines vs. pick one~~ — **DECIDED: union** (near-zero overlap across the whole golden pass; best entities were dual-engine hits).
- ~~Net-new ingest cap~~ — **DECIDED: K = 6**, selected by relevance score with the 0.5 band included (golden-pass grading, 2026-08-18).
- ~~Resolver design~~ — **DECIDED 2026-08-18: two-layer de-dupe, no standalone graph-probe resolver.** Layer 1: LLM removes response-node duplicates from the entity array (folded into the relevance gate). Layer 2: the idempotent get-or-create seed gates provide Universe-wide de-dupe as a side effect. The registry's `graph_probe` column remains as QA documentation, not a runtime dependency.
