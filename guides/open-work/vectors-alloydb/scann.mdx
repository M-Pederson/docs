...
```mermaid
graph TB
    A["<b>Vector Database</b><br/>AlloyDB + vector extension"] --> B["<b>Input Vectors</b><br/>3072-dim embeddings (gemini-embedding-001)<br/>name_embedding, expertise_embedding"]
...
_Figures below are illustrative at 384-dim. Orbiter's production vectors are **3072-dim** (`gemini-embedding-001`), so raw vector storage scales ~8× — budget **~12 GB per 1M vectors per column** before the index. ScaNN's quantized index stays far smaller than the raw vectors._
