...
<Note>
**Session paused here - June 26, 2026.** Frontend work is committed on `orbiter-frontend` branch `ai-sdk-chat` at `dd246877` (`Refactor Orbiter Copilot chat for AI SDK`), with a clean worktree. The user logged into local `http://127.0.0.1:3000` with the live-linked account and confirmed all live outcomes are visible. Verification completed after the commit: targeted tests passed (`6` files, `119` tests), `git diff --check` passed, and `pnpm smoke:copilot-ai-sdk http://127.0.0.1:3000 disabled` passed with both `default` and `leverage` returning `status: "disabled"`. We intentionally did **not** send a live Copilot chat turn, because that can create/persist real conversation data. Next resume step: add `VITE_COPILOT_AI_SDK=true` and server-only `OPENROUTER_API_KEY` to the gitignored frontend `.env`, restart the dev server on port `3000`, verify only that the key is present, then run the flag-on AI SDK smoke/default streaming check.
</Note>
