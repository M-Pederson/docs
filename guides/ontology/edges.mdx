...
| **Writer** | `mvp/node/sync-domain-expertise-nodes` (#12663) — relational→graph sync for `sub_domain_expertise` rows missing a `node_uuid` — alongside `mvp/expertise/attach-subdomain-parent` (#13142) — forward-path parent attach for SubDomain nodes freshly created by `resolve-company-specialties` (#12746), parent picked by the #13141 LLM resolver |
...
The forward-path writer (#13142) `MERGE`s with `{weight: 100}`; the §3.6 audit set the same explicit `100` on legacy null-weight rows.
...
#12701 materializes the `Funding_Round` node from `fundable_deals` + writes the company-side `RAISED` edge. #12702 walks every institutional investor + angel on the deal and writes investor-side edges (`LEAD_INVESTED_IN`, `INVESTED_IN`, `FOLLOW_ON_INVESTED_IN`, `INVESTMENT_PARTNER_IN`, `INVESTMENT_PARTNER_AT`). #12856 (v1.6, 2026-07-02) is the single source of truth that orchestrates both — called from `add-all-fundable-deals` (#12703 v2.1, target side) and `process-person-phase-9` (investor side). **Repaired v1.12 (2026-07-02, sandbox-verified):** #12702 reads `cascade_depth` as a nullable input via `first_notnull`, so seed-deal runs write the full investor edge set below; from v1.11 (2026-06-23) until the fix a `first_notempty` depth coercion made the writer self-skip on every invocation — deals processed in that window carry `Funding_Round` \+ `RAISED` only. See [Company Edges](/guides/enrichment/waterfall/company-edges).
...
### Integer scale (5–100, lower = stronger)
...
| `PLAYED_ON` | **20** |
| `ENGINEERED` | **22** |
| `HAS_EXPERTISE` | 10-50 (dynamic — `min(round(10 + match_score×80), 50)`) |
