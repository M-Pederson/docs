...
- ~~**Tier-3 CREATE-CONTACTS with nothing to copy.**~~ **Dissolved 2026-08-08 by the [Edge Sync](/guides/open-work/edge-sync) reference-model decision** — CREATE-CONTACTS never copies a subgraph; it creates a stub + KNOWS edge from the GET-ADD response alone, which works identically for brand-new and fully-enriched people.
- ~~**Enrichment-update fan-out to user graphs.**~~ **Resolved 2026-08-08 by the [Edge Sync](/guides/open-work/edge-sync) decision** — user graphs never copy shared edges, so there is nothing to fan out; enrichment updates are visible to every user's next composed query the moment Universe writes them. The only surviving propagation is the merge/node-event channel (stub repointing on Universe merges).
- ~~**Is the user-graph copy a duplicate or a reference?**~~ **Decided 2026-08-08 — reference, on [Edge Sync](/guides/open-work/edge-sync)**: user graphs hold only the private layer (thin stubs by `node_uuid` + KNOWS/FOLLOWS/WORKS_AT + strength + notes/files); all shared topology stays in Universe; reads compose the layers app-side. CREATE-CONTACTS step 3 accordingly shrinks from "copy subgraph" to "create stub + KNOWS edge."
...
- **What do inbound emails do?** Contact creation is outbound-only (decided). Presumably an inbound email from an existing tier-1 contact still LOG-INTERACTIONs (a reply from a contact is relationship evidence), and inbound from an unknown sender is ignored entirely — confirm both halves. **Partial update 2026-08-08 ([Edge Sync](/guides/open-work/edge-sync) FOLLOWS-gate decision): meeting participation — attending/accepting a calendar event with a person — is now the second contact-creating intent event alongside outbound email (small-meeting cap applies). Passive inbound remains a no-op.**
