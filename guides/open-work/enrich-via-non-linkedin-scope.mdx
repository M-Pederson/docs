...
<Warning>
  Net result: a **junk-named, enrichment-less node** seeds the graph from a bare social handle, and nothing flags it as low-quality. This is exactly the *"Verify master_person creation from a non-LinkedIn profile URL"* item on the [migration freeze checklist](/guides/open-work/enrichment-gcp-migration).
</Warning>
...
The crux is **what the scraped name is matched *against*.** Two modes:
...
**Recommendation:** *require a name to proceed.* If there's neither a caller-provided name nor a confidently-resolved real name, **decline** rather than create — that's the whole point of removing the `temp_` path.
...
- **Does this also apply when a name *is* provided** (verify-before-create) **or only the no-name case?**
- **Where to build it** — a pre-migration Xano change, or a behavior in the [Universe Go rebuild](/guides/open-work/enrichment-gcp-migration)? Given the parity-first / hard-freeze stance, **scoping now and building it in the Go rebuild** avoids adding to the frozen Xano baseline.
...
- **Related:** [Enrichment GCP Migration](/guides/open-work/enrichment-gcp-migration) · [Person enrichment phases](/guides/enrichment/waterfall/person-enrichment-phases) · [Person enrichment cascade](/guides/open-work/person-enrichment-cascade)
