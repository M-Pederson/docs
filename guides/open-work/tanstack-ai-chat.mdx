...
| Phase 11 - Cutover And Rollback Window | Ready | 2026-06-28 | Cutover/rollback runbook created in `dogfood-output/tanstack-ai-chat-phase11/cutover-runbook.md` and PR-readiness checklist created in `dogfood-output/tanstack-ai-chat-phase11/pr-readiness-checklist.md`; Phase 10 core chat/final dispatch QA is passed, rollback remains per-surface through `VITE_TANSTACK_AI_CHAT_HOW_I_KNOW`, `VITE_TANSTACK_AI_CHAT_MEETING_PREP`, `VITE_TANSTACK_AI_CHAT_LOOPS`, and `VITE_TANSTACK_AI_CHAT_OUTCOMES`, no PR has been opened, no production deploy flags have been changed, and no legacy code has been removed. | Continue manual QA, then execute the runbook in the intended deploy environment later: open PR, enable flags in proven rollout order, capture production/dogfood traces after each surface, and rollback-smoke at least one surface before closing the window. |
