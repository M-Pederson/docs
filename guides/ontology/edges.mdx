...
MusicBrainz edges are produced by the [Music Industry Waterfall](/guides/enrichment/waterfall/music-industry-waterfall). The full implementation reference, including the standard edge envelope and description-generation pass, lives on [Music Edges](/guides/enrichment/waterfall/music-edges); this section keeps the ontology edge inventory complete.

<Note>
`Artist` below means either `Person:Music_Artist` or `Music_Group`. `MEMBER_OF`, `PLAYED_ON`, `PRODUCED`, and `ENGINEERED` are person-specific where noted. Genres remain node properties, not `HAS_GENRE` edges.
</Note>
...
Every music edge carries `uuid`, `weight`, `created_at`, `updated_at`, `description`, and `description_generated_at`, plus the edge-specific properties below.

</Card>

### Membership & artist relationships

| Edge | Pattern | Weight | Edge-specific properties |
| --- | --- | ---: | --- |
| `MEMBER_OF` | `(Person:Music_Artist)→(Music_Group)` | **5** current / **12** past | `instruments`, `begin`, `end`, `ended`, `original` |
| `COLLABORATED_WITH` | `(Artist)↔(Artist)` | **15** | `project`, `begin`, `end` |
| `SUPPORTING_MUSICIAN_FOR` | `(Person:Music_Artist)→(Artist)` | **18** | `instruments`, `vocal` |
...
| Edge | Pattern | Weight | Edge-specific properties |
| --- | --- | ---: | --- |
| `COMPOSED` | `(Artist)→(Work)` | **8** | — |
| `WROTE_LYRICS` | `(Artist)→(Work)` | **12** | — |
| `WROTE` | `(Artist)→(Work)` | **10** | `role` |
| `PERFORMED` | `(Artist)→(Recording)` | **10** | `guest` |
| `PLAYED_ON` | `(Person:Music_Artist)→(Recording)` | **20** | `instruments`, `vocal` |
| `PRODUCED` | `(Person:Music_Artist)→(Recording)` | **18** | — |
| `ENGINEERED` | `(Person:Music_Artist)→(Recording)` | **22** | `role` |

### Releases & labels

| Edge | Pattern | Weight | Edge-specific properties |
| --- | --- | ---: | --- |
| `RECORDING_OF` | `(Recording)→(Work)` | **30** | `live`, `partial`, `cover` |
| `RELEASED` | `(Artist)→(Release_Group)` | **10** | `joinphrase` |
| `RELEASE_OF` | `(Release)→(Release_Group)` | **30** | — |
| `APPEARS_ON` | `(Recording)→(Release)` | **45** | `track_number`, `position`, `disc` |
| `RELEASED_ON` | `(Release)→(Company:Music_Label)` | **50** | `catalog_number` |
| `ON_RECORD_LABEL` | `(Recording)→(Company:Music_Label)` | **55** | `via_release`, `catalog_number` |
| `SUBSIDIARY_OF` | `(Company:Music_Label)→(Company:Music_Label)` | **15** | `begin`, `end` |

`ON_RECORD_LABEL` is derived in-graph from `(Recording)-[:APPEARS_ON]->(Release)-[:RELEASED_ON]->(Company:Music_Label)` by `mvp/music/backfill-on-record-label-edges` (#13131); `RELEASED_ON` remains the source-of-truth release-to-label relationship.

### Events, places & geography

| Edge | Pattern | Weight | Edge-specific properties |
| --- | --- | ---: | --- |
| `HEADLINED` | `(Artist)→(Event:Music_*)` | **35** | — |
| `PERFORMED_AT` | `(Artist)→(Event:Music_*)` | **45** | `role` |
| `HELD_AT` | `(Event:Music_*)→(Place)` | **30** | — |
| `RECORDED_AT` | `(Recording)→(Place)` | **35** | — |
| `FROM_AREA` | `(Artist / Company:Music_Label / Place)→(Country / Region / City)` | **70** | `area_type` |

### Identity & ownership bridges

| Edge | Pattern | Weight | Edge-specific properties |
| --- | --- | ---: | --- |
| `SAME_AS` | `(Company:Music_Label)→(Company)` | **1** | `match_method`, `confidence` |
| `OWNED_BY` | `(Company:Music_Label)→(Company)` | **15** | `match_method`, `confidence`, `source` |

`SAME_AS` is label-to-company reconciliation only; `Person:Music_Artist` uses the shared `Person` label and music person resolution instead of an IMDB-style person card or separate person card type.
