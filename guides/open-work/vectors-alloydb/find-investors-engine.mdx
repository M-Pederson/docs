...
- **`pplx_embeddings`** (VECTOR) — *AlloyDB-only.* Full-document embedding of `markdown` via `perplexity/pplx-embed-v1-4b`. **Used on `file` rows only in Orbiter** — the `investment_thesis` and `fundraising_pitch_profiles` tables embed with `openai/text-embedding-3-small` instead. Verify the output dimensionality with OpenRouter before applying the migration (placeholder `VECTOR(1536)` below).
...
**Indexes** — the four btree indexes from the Xano source (`user_id`, `created_at DESC`, `file_source`, `mime_type`) plus two AlloyDB-only: `file_type` btree (so the pipeline can filter pitch decks fast) and HNSW on `pplx_embeddings` for full-document cosine similarity search.
...
**New to unstructured.io?** It's a document parser (service + OSS library) that takes any file — PDF, DOCX, PPTX, HTML, plain text, scanned images via OCR — and breaks it into a stream of typed *elements*. A heading becomes a `Title`, a paragraph becomes `NarrativeText`, a bullet is `ListItem`, a table is `Table` (with row/column structure preserved as HTML), images arrive with optional captions / OCR text. Each element carries metadata: source page number, bounding box coordinates, and parent-child links for nested structure. unstructured.io also produces a joined plain-text rendering of the whole document — that joined output is what gets written to `file.markdown` upstream. This `file_element` table stores the element-level stream alongside the joined markdown, so downstream features can reach for structure (page jumps, table-aware extraction, slide-by-slide analysis) without reparsing.

Sibling to the `file` table, joined on `file_id`. The pitch pipeline (`pitch/extract-pitch-profile`) reads only `file.markdown` and is unchanged by this table — element rows are additive, unlocking opt-in upgrades without forcing a migration.

**Note for devs — element ≠ chunk.** An *element* is a parsed primitive from unstructured.io (one Title, one paragraph, one Table). A *chunk* is a derived, retrieval-sized grouping of elements with overlap, used for vector retrieval at the passage level. We don't chunk yet (see "When to add chunking" at the bottom of this box). Don't conflate the two — `file_element` is structure-from-the-parser, not pre-chunked retrieval units.

**Element types from unstructured.io** — `Title`, `NarrativeText`, `ListItem`, `Table`, `Image`, `Header`, `Footer`, `FigureCaption`, `Address`, `Formula`, `CodeSnippet`, `PageBreak`, `UncategorizedText`. Stored as TEXT, no enum CHECK constraint — so unstructured.io adding a new type in a release doesn't force a migration.
...
**Cascading delete** — `ON DELETE CASCADE` on `file_id` means deleting a `file` row sweeps its `file_element` rows automatically. No orphan-cleanup task needed.

**Reassembling joined markdown** — `SELECT text FROM file_element WHERE file_id = $1 ORDER BY element_index` returns the document in source order. The pipeline writes the joined output to `file.markdown` directly so this query isn't required today; it's the fallback if joined storage is ever derived rather than persisted.

---

**Future opt-in enhancements** (additive — none required for v1, none force a migration of existing rows)
...
**When to add chunking** (for much larger docs — research reports, books, multi-hundred-page PDFs)

The current schema embeds the whole `file.markdown` as one `pplx_embeddings` vector. That works as long as the markdown fits inside `perplexity/pplx-embed-v1-4b`'s context window (verify the actual limit on OpenRouter). Add chunking when one of these triggers fires:

- Uploads regularly exceed the embed-model context window (truncation starts losing signal).
- A feature requires passage-level retrieval ("jump to the page that mentions X", citation grounding, RAG over uploaded files).

When that day comes, add a sibling `file_chunk` table — pure additive migration, no rewrite of `file` or `file_element`:
...
- Embedding model can match `file.pplx_embeddings` for consistency or switch to the workspace default `openai/text-embedding-3-small` — pick per use case.
