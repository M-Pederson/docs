...
| --- | --- | --- | --- | --- |
| 2363 | `clean-convo/llm_clean_messages` | Route to a cleaner, store `clean_message` | none — delegates | `message.created` webhook, historical parse |
| 4647 | `clean-convo/clean-email-body` | Clean one email body (reply/plain) | `openai/gpt-5-nano` | via #2363 |
| 12648 | `clean-convo/clean-email-body-forward` | Clean a forwarded email body | `openai/gpt-5-nano` | via #2363 — apparently never routed, see observations |
| 12644 | `mvp/activity/email-llm-summary` | Thread → summary/bullets/sentiment/actions JSON | `google/gemini-3.1-flash-lite-preview` | `message.created` webhook, historical parse |
...
| --- | --- | --- |
| `email_activity.clean_message` (table 597) | #2363 after each cleaner run | `mvp/nylas/enrich-messages` #12649 for inbox display; merge logic |
...
| `email_ai_summary` | #12644 upsert keyed `(grant_id, thread_id)` | EmailSummary endpoints (Go backend) |
...
- **Live inbound mail:** the `message.created` webhook (#180) helper filters drafts/spam/promotions, creates activity rows, and — only when `nylas_grant.ai_enabled` — fires #2363 and #12644 async. See [Message Created](/guides/log-parse/message-created).
...
- **Historical import:** `mvp/activity/nylas-history-parse` (#189) runs an AI stage per recent message (`get-message` → #2363) and per unique thread (#12644); the account-import path fires #10518 for newly added contacts and #10519 to apply stored signatures. See [Historical](/guides/log-parse/historical).
...
Verified while reading the live bodies for this inventory — logged here for the review, not yet actioned:

1. **Five separate cleaning implementations** exist: the two clean-convo LLM cleaners (each with its own JS pre-clean), #12644's inline lambda, the Mem0 extractors' inline lambda, and the shared extraction lambda duplicated in #4645/#10518. The Mem0 design doc already flags consolidating on one shared cleaner.
2. **#2363's forward routing appears dead.** The detect lambda reads `$var.body`, but the value only exists as `$input.body` — so `is_forward` is always false and every message routes to the base cleaner. #12648 (and its discarded `new_text`) would never run from this path.
3. **#2363 has no processed-guard** — every invocation re-fetches nothing but re-runs the LLM clean and overwrites `clean_message` on all matching rows.
4. **#12644 returns the pre-write row** — first-ever run returns empty, re-runs return the previous summary. It also fetches `ai_enabled` without checking it (gating happens upstream) and has no try/catch around the Nylas or OpenRouter calls.
5. **#12644 re-fetches and re-cleans from Nylas** even when a stored `clean_message` already exists for the messages — cleaned output is not reused across features.
...
8. **#10518 embeds a live Nylas API key literal** in a disabled debug block — should be removed and the key rotated.
