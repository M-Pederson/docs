...
Mirrors the Xano workspace 3 `file` table (id `587`) with **four columns called out for the AlloyDB ingest pipeline**: `markdown`, `llm_description`, `file_type`, and `pplx_embeddings`. The Qdrant-specific columns from the Xano source (`chunk_count`, `qdrant_collection`) are dropped — AlloyDB pgvector replaces Qdrant.

**Pipeline-relevant columns**

- **`markdown`** (TEXT) — extracted by unstructured.io during ingest (Save File Pipeline step 2). Already present on the Xano source; preserved here because it's the input to both `classify_file` and `pplx_embeddings`.
...
**Dropped from Xano source** — `chunk_count` and `qdrant_collection`. Qdrant is replaced by AlloyDB native pgvector; chunk-level vectors (if needed later) can live in a separate `file_chunk` table referencing `file.id`.
