...
<Note>
  **Live prototype (2026-08-15):** the person-side build now exists as `persona/build-persona` (#13349, v0.6) — see [**Persona Build Design**](/guides/open-work/personas/persona-build-design) for the working prompt, the `persona` (754) \+ `compare_persona` (778) \+ `queue_persona` (779) tables, model bake-off results (kimi-k3 primary \+ Claude Sonnet 5 retry), and an **experimental ninth dimension, `current_needs`**, proposed for this canonical set at promotion. The 8-dimension lock below stands until that decision.
</Note>
...
<div style={{padding: '20px', border: '2px solid #6B9FE8', borderRadius: '8px', color: '#fff', margin: '16px 0'}}>
  ...
  * **Track B — Personas (this page)** sits *on top* of Track A. The person-persona rollup function reads from `person_has_expertise` (and other enrichment sources) to build the per-person 8-dimensional profile. **Live as the workshop prototype `persona/build-persona` #13349 (see Note above); canonical `person/build-persona` lands at promotion.**
...
| 2 | `domain_focus` | What do they _know_? | `HAS_EXPERTISE` rows (Track A) \+ LLM industry/niche narrative |
| 3 | `soft_skills` | What can they _do_ operationally? | LLM-synthesized from social signals, recommendations, peer mentions, evidence in track record (e.g. "led 50-person team" → leadership) |
...
| 8 | `passions_interests` | What do they care about _beyond work_? | Causes, hobbies, side projects, advocacy, personal projects |

**Reads as a narrative:** _who → know → can do → have done → talk about → how → who with → what beyond work._
...
2. **`outcome/extract-ideal-persona`** — DeepSeek-v3.2 via OpenRouter, JSON output, one block per dimension. Returns 8 narratives \+ initial dimension\_weights \+ initial filter constraints.
...
- `industry_tags` — from companies in work history \+ LLM extraction from domain\_focus narrative
...
The `vectors/create-gemini-vectors` function (#13114) is the Gemini `gemini-embedding-001` wrapper (1536-d L2-normalized since v2 2026-07-06 — was 3072-d; the 3072-d vector specs throughout this page predate the change and any Gemini embed built from them must use 1536), used across the node and expertise embedding writers. Note the as-built orchestrator #12928 embeds via `vectors/create-vectors-string` #4676 (OpenAI, 1536) — not Gemini — so the stored persona vectors are 1536-d today.
...
The matcher SQL function takes an `outcome_id`, reads its 8 vectors \+ filter arrays \+ dimension\_weights, runs 8 parallel ScaNN top-K lookups (each filtered inline), unions the candidate pool, then exact-rescores against all 8 dimensions and returns top-K with per-dimension breakdown. Detailed query template will live in this page once Personas tables are populated and the AlloyDB migration is in flight.
...
| `person/build-persona` rollup | ⚠️ corrected 2026-08-15: was **never built** (this row previously claimed done) | Superseded by workshop `persona/build-persona` **#13349** (v0.6) → persists to `persona` table **754**; rename to `person/build-persona` at promotion — see [Persona Build Design](/guides/open-work/personas/persona-build-design) |
| `passions_interests` LLM cut | ✅ covered by the workshop build | Fed by `master_person.interests` (social-insights v2/v3 \+ deep-bio writers) \+ deep-bio payload; dedicated extract fn not needed so far |
| Pairwise compare cache | ✅ built 2026-08-15 | `compare_persona` table **778** \+ `persona/compare-personas` **#13350** — canonical-pair cache, per-dimension cosine, lazy 1536-d embedding backfill into 754 |
| `queue_persona` production queue | ✅ table built 2026-08-15 | **id 779** — unique on `master_person_id`, upsert-only per house queue convention. Feeds the production POST trigger \+ CRON drain design (endpoint and CRON **not yet built**) — see the callout on [Persona Build Design](/guides/open-work/personas/persona-build-design) |
