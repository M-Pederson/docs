...
The current schema embeds the whole `file.markdown` as one `pplx_embeddings` vector. `perplexity/pplx-embed-v1-4b` has a **32,000-token context window** (~128K characters of markdown). Most pitch decks, invoices, resumes, articles, and meeting notes are well under that — chunking isn't worth the complexity for v1.
