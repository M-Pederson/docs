...
- **Hub targets sit in the Moderate band or below.** Institutions, committees and conferences connect thousands of people, so their edges never compete with the structural bonds (`FOUNDED` 5, family 1–10) — the 2026-05-31 audit rule.
...
Every edge carries the 2026-07-04 envelope (`uuid`, `created_at`, `updated_at`, `weight`, `description`, `description_generated_at`); `description` is the deterministic sentence given per edge. Property tables below list only the edge-specific fields.
...
| Foundation → institution | **30**; **40** past the grant-count hub threshold | Mega-grantmakers are donor-side hubs (the `LOCATED_IN` 70/80 precedent) |
