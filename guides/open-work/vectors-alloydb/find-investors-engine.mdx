...
```sql
-- Inside Cloud Run, after embedding the 6 pitch narratives:
SELECT * FROM match_investors_for_pitch(
  $1::vector(1536),  -- founder_fit
  $2::vector(1536),  -- problem_market
  $3::vector(1536),  -- competitive_moat
  $4::vector(1536),  -- traction_momentum
  $5::vector(1536),  -- business_model
  $6::vector(1536),  -- expansion_roadmap
  10,                                            -- top_n
  '["seed","series_a"]'::jsonb,                  -- p_stage_filter
  NULL                                           -- p_geo_filter (no restriction)
);
```

Result (truncated to top 5, scores rounded to 2 decimal places for readability):
...
-------------------+---------+------+------+------+------+------+------+-----------
 ...
 First Round       | vc_fund | 0.86 | 0.74 | 0.71 | 0.72 | 0.81 | 0.68 |     0.77
 Initialized Cap.  | vc_fund | 0.79 | 0.76 | 0.75 | 0.68 | 0.74 | 0.65 |     0.74
...
The same data, as the Cloud Run service returns it to the Xano caller (which forwards it to the founder UI):
...
The `explanation` array is generated by Cloud Run, not the SQL function — it picks the top 2-3 highest-scoring dimensions per investor and formats a one-line reason from the matching narrative excerpt. Lets the founder UI render *why* each investor scored where it did, not just *that* it scored.
...
All 6 HNSW index scans run before the union/hash-aggregate; with `PARALLEL SAFE` set on the function and `max_parallel_workers_per_gather >= 4`, AlloyDB will fan them out across worker processes — the bulk of the 287ms is the 6 HNSW lookups happening in parallel, not in series.
