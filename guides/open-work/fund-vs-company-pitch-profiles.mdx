...
<Note>
  **Status (2026-09-04).** Built on branch `ORBIT-1852/optimize-pitch-profile-for-funds` in `orbiter-backend` for Linear ticket ORBIT-1852. Not yet merged; migration `files_011_fund_profiles` not yet applied in dev. Scoring is expected to move to Universe; the `fund-thesis-match` endpoint documented below is kept in the backend as the reference implementation to port from. `outcomes.investment_theses*` is unchanged.
</Note>
...
Go-side `normalize()` in `fundprofile/extractor.go` drops a quality score outside 0 to 100 and any provider role with an empty value. Both were observed live: deepseek returned `-75` and `-1` for two of four decks on the first run, and filled unnamed roles with `""`.
...
1. **Read the per-dimension scores, not just the composite.** A match that ranks high on `gp_track_record` alone may be an angel whose `founder_fit` narrative is long and generic. If that pattern shows up across many results, lower that weight.
2. **Expect `fund_economics` to be noise.** An investor's `business_model` derived narrative describes the GTM patterns of their portfolio companies; a fund's fee and carry terms have nothing to say to it. The 0.03 weight is there so the dimension is not silently discarded, not because it carries signal. Dropping it to zero is reasonable.
3. **`fund_thesis` versus `problem_market` is the pairing to trust first.** Sector, stage and geography language is where the two sides actually talk about the same thing.
4. **Consider the declared thesis too.** The backend matches only against `source = 'derived'` (what the investor's portfolio reveals). For funds, an investor's `declared` narrative (what they say they look for) may be the better target for `fund_thesis`, since LP-style investors describe their mandate more than they reveal it through deals.
5. **Use `investor_type` before touching weights.** If fund results are dominated by angels who never write fund commitments, the fix is a filter, not a weight.
6. **Structured boosts are not in v1.** `fund_profiles.industries`, `stage_focus` and `geography` are named to match the thesis columns so an exact-overlap boost on top of cosine is a small addition once the ordering from cosine alone has been reviewed.
7. **Try alternative pairings.** Because labels are just join keys, Universe can test `performance_momentum` against `founder_fit` (repeat-exit operators) or `sourcing_edge` against `problem_market` without any backend change.

## Live extraction results (dev decks)

Run with the final prompt against the stored markdown of the four dev fund decks. Times are the extractor call only.
...
- Apply `files_011` in dev (CI runs `goose up` on deploy). Re-timestamp the migration before merge if `main` gains newer migrations.
- Backfill the four dev fund decks: they are still `classification = fundraising_deck` with `pitch_profiles` rows. Reclassify, delete those rows, republish `fund_profile.requested`.
- Build theses for LP-type investors (`family_office`, `fund_of_funds`) so `investor_type` filtering has something to select.
...
- Tune the fund weights once the first real match scores are visible.
