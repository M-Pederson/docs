...
#12701 materializes the `Funding_Round` node from `fundable_deals` + writes the company-side `RAISED` edge. #12702 walks every institutional investor + angel on the deal and writes investor-side edges (`LEAD_INVESTED_IN`, `INVESTED_IN`, `FOLLOW_ON_INVESTED_IN`, `INVESTMENT_PARTNER_IN`, `INVESTMENT_PARTNER_AT`). #12856 (v1.5, 2026-06-24) is the single source of truth that orchestrates both — called from `add-all-fundable-deals` (#12703 v2.1, target side) and `process-person-phase-9` (investor side). **As-built (flagged, 2026-07-02 live audit):** since #12702 v1.11 (2026-06-23) a `first_notempty` depth-guard coercion makes the investor-edge writer **self-skip on every live invocation** — live runs currently produce only `Funding_Round` \+ `RAISED`; the investor edges below are the as-written contract. See [Company Edges](/guides/enrichment/waterfall/company-edges).
