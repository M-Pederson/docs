...
3. **Coverage de-weighting (optional refinement).** The `synthesis` coverage grades (`rich | moderate | thin`) exist for match time: shrink `thin` dims toward 0 before aggregating. `dims_compared` doubles as a free UI signal ("matched on 6 of 9 facets").
...
Production surfaces **24** results per loop run. The recommended composition is a **portfolio of four shelves** — mode × lane: peer-diagonal ("people like you"), peer-serendipity, complement-diagonal ("the missing piece"), complement-serendipity — **6 slots each, with per-shelf quality floors and backfill**, deduped so each candidate appears only on their best shelf. Both modes are pure arithmetic over the same saved scores, so the portfolio costs nothing extra. Full mechanics and the simpler blended-list fallback: [developer-requirements](/guides/open-work/suggestion-recipes/developer-requirements#composing-the-top-24).
