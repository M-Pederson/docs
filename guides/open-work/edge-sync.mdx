...
| `WORKS_AT` | Company | User's own current employment (`current: true`) |
| `ATTENDED` | School | User's own education (self-enrichment) |
...
**Content edges** (User → …): `CREATED` → Note · `RECEIVED` → Note (shared in) · `UPLOADED` → File (manual) · `SAVED` → File (auto-captured from email) · `COLLECTED` → any entity (collection membership; `collection_id`, `added_at`, `source: own | shared_collection` + sharer)
...
1. **New user sign-up** — all direct-connect nodes attach (current companies via WORKS_AT, education via ATTENDED, imported/connected people); **expertise nodes are excluded** (they stay Universe-side).
...
5. **Contacts import** (vCard / CSV / …) → CREATE-CONTACT-PERSON.
