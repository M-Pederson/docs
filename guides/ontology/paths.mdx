...
WHERE NONE(node IN nodes(p)[1..-1] WHERE node['display_label'] IN
  ...
  AND NONE(rel IN relationships(p) WHERE type(rel) IN
        ['LOCATED_IN', 'IN_REGION', 'IN_CITY', 'IN_COUNTRY', 'IN_STATE',
         ...
         'BORN_IN', 'FROM_LOCATION', 'HAS_LOCATION'])
...
- **Geo exclusion** — intermediate nodes may not be geographic (`City`/`Region`/`Country`/`State`/`Continent`/`Location`) and no edge on the path may be a location edge, so "you both live in LA" never counts as a connection.
