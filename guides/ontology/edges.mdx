...
**Weight convention:** Lower numbers indicate stronger / more important relationships. IMDB film/TV edges use a 0.0–1.0 scale; all other edges use an integer 5–100 scale.
...
**v5 / v2 chain (2026-05-17).** Person-side: LLM identifies domains of expertise via `mvp/expertise/llm-identify-person-expertise` (#12666), which calls #12926 for each. #12926 vector-searches against existing `Entity:SubDomainExpertise` / `Entity:DomainExpertise` nodes, creates a new node if no match, and `MERGE`s the `HAS_EXPERTISE` edge with persisted `r.weight`. Successful graph MERGE triggers #12925 to dual-write into `person_has_expertise` (Xano table 711). Companies use a parallel path: `mvp/expertise/resolve-company-specialties` (#12746) writes `SPECIALIZES_IN`. The hierarchical `HAS_SUBDOMAIN` edge is written by the relational sync (#12663), forward-path parent attach (#13142), and taxonomy seeder (#13161); all three now stamp the full edge metadata envelope.
